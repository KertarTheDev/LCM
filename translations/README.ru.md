...
### Предварительный выпуск KiloCode-LCM

KiloCode-LCM — это модифицированная версия Kilo Code, созданная для лучшей работы с длинными сессиями.

Обычные агенты для программирования рано или поздно упираются в лимит истории чата. Когда это происходит, они обычно сжимают старую переписку в короткое резюме и теряют детали. Чат может продолжаться, но агент может забыть точные команды, файлы, ошибки и решения из более ранних этапов задачи.

LCM работает иначе. Он хранит локальную память разговора, при необходимости резюмирует старые части и дает агенту безопасные способы позже снова найти детали. Цель простая: длинные сессии должны меньше ощущаться как новый старт после каждого лимита контекста и больше как работа с тем, кто может заглянуть в свои заметки.

Что это должно улучшить:

- Лучшую непрерывность в длинных задачах по коду.
- Меньше случайного забывания, когда контекст становится большим.
- Более надежное восстановление точных ранних деталей.
- Меньше случаев, когда старый контекст тихо выбрасывается.

Расход токенов зависит от задачи. Короткие чаты могут использовать примерно столько же токенов или немного больше, потому что LCM держит свежий недавний контекст и может выполнять фоновую работу с памятью. Длинные сессии обычно должны тратить меньше токенов на постоянное повторное включение старой истории чата в основной prompt, но часть этой экономии может уходить на обслуживание памяти или запросы поиска. На практике ожидайте, что LCM сначала оптимизирует более надежное запоминание, а уже потом уменьшение потерь long-context токенов.

Это все еще предварительный выпуск. Для тестирования LCM используйте артефакты GitHub Releases из этого репозитория. Обычные пакеты Kilo Code в Marketplace и npm остаются upstream-релизами.

LCM вдохновлен статьей [LCM: Lossless Context Management](https://arxiv.org/abs/2605.04050). Больше деталей реализации доступно в [спецификациях текущего кода](specifications/README.md).
