...
KiloCode-LCM은 긴 세션을 더 잘 처리하도록 설계된 Kilo Code의 수정 버전입니다.

일반적인 코딩 에이전트는 결국 채팅 기록을 담을 공간이 부족해집니다. 그럴 때 보통 오래된 대화를 짧은 요약으로 압축하고 세부 내용은 버립니다. 이렇게 하면 채팅은 계속할 수 있지만, 에이전트가 이전의 정확한 명령, 파일, 오류, 결정 사항을 잊어버릴 수 있습니다.

LCM은 다르게 작동합니다. 대화의 로컬 메모리를 유지하고, 필요할 때 오래된 부분을 요약하며, 나중에 세부 정보를 안전하게 다시 찾아볼 수 있는 방법을 에이전트에 제공합니다. 목표는 단순합니다. 긴 세션이 매번 컨텍스트 한도에 도달할 때마다 새로 시작하는 느낌이 아니라, 필요하면 메모를 다시 확인할 수 있는 사람과 함께 일하는 느낌에 가까워지는 것입니다.

기대되는 개선점:

- 긴 코딩 작업에서 더 나은 연속성.
- 컨텍스트가 커졌을 때 우발적인 망각 감소.
- 이전의 정확한 세부 정보에 대한 더 신뢰할 수 있는 복구.
- 오래된 컨텍스트가 조용히 버려지는 경우 감소.

토큰 사용량은 작업에 따라 달라집니다. 짧은 채팅은 LCM이 최근 컨텍스트를 신선하게 유지하고 백그라운드 메모리 작업을 할 수 있기 때문에 거의 같은 수의 토큰을 쓰거나 약간 더 쓸 수 있습니다. 긴 세션은 일반적으로 오래된 채팅 기록을 메인 프롬프트에 반복해서 담는 데 쓰는 토큰이 줄어들어야 하지만, 그 절약분의 일부는 메모리 유지 관리나 조회 호출에 사용될 수 있습니다. 실제로는 LCM이 먼저 더 안전하게 기억하는 것을, 그다음으로 긴 컨텍스트 토큰 낭비를 줄이는 것을 최적화한다고 보면 됩니다.

이 버전은 아직 프리릴리스입니다. LCM을 테스트할 때는 이 저장소의 GitHub Release 아티팩트를 사용하세요. 일반 Kilo Code Marketplace 및 npm 패키지는 계속 upstream 릴리스입니다.

LCM은 논문 [LCM: Lossless Context Management](https://arxiv.org/abs/2605.04050)에서 영감을 받았습니다. 더 자세한 구현 내용은 [current-code specifications](specifications/README.md)에서 확인할 수 있습니다.
