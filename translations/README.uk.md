...
### Попередній випуск KiloCode-LCM

KiloCode-LCM — це модифікована версія Kilo Code, створена для кращої роботи з довгими сесіями.

Звичайні агенти для програмування зрештою впираються в обмеження історії чату. Коли це трапляється, вони зазвичай стискають стару розмову в короткий підсумок і викидають деталі. Це дозволяє продовжити чат, але агент може забути точні команди, файли, помилки та рішення з попередніх етапів задачі.

LCM працює інакше. Він зберігає локальну пам'ять розмови, за потреби підсумовує старі частини та дає агенту безпечні способи пізніше знову знайти деталі. Мета проста: довгі сесії мають менше відчуватися як початок з нуля після кожного ліміту контексту, і більше як робота з кимось, хто може переглянути свої нотатки.

Що це має покращити:

- Кращу безперервність у довгих задачах із кодом.
- Менше випадкового забування, коли контекст стає великим.
- Надійніше відновлення точних попередніх деталей.
- Менше випадків, коли старий контекст тихо відкидається.

Використання токенів залежить від задачі. Короткі чати можуть використовувати приблизно стільки ж токенів або трохи більше, бо LCM тримає свіжим недавній контекст і може виконувати фонову роботу з пам'яттю. Довші сесії зазвичай мають витрачати менше токенів на постійне перенесення старої історії чату в основний prompt, але частина цієї економії може піти на обслуговування пам'яті або пошукові виклики. На практиці очікуйте, що LCM спершу оптимізує безпечніше запам'ятовування, а вже потім зменшення марних long-context токенів.

Це все ще попередній випуск. Для тестування LCM використовуйте артефакти GitHub Releases із цього репозиторію. Звичайні пакети Kilo Code у Marketplace та npm залишаються upstream-релізами.

LCM натхненний статтею [LCM: Lossless Context Management](https://arxiv.org/abs/2605.04050). Більше деталей реалізації доступно в [специфікаціях поточного коду](specifications/README.md).
