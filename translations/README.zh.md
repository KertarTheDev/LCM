...
KiloCode-LCM 是 Kilo Code 的一个修改版，目标是改进长会话的处理方式。

普通的编码代理最终会用完聊天历史的空间。发生这种情况时，它们通常会把较早的对话压缩成一小段摘要，并丢掉细节。这样聊天还能继续，但代理可能会忘记任务早期的精确命令、文件、错误和决定。

LCM 的做法不同。它会在本地保存对话记忆，在需要时总结较早的部分，并给代理提供安全的方法，让它之后可以重新查找细节。目标很简单：长会话不应该像每次到达上下文限制后都重新开始，而应该更像是在和一个还能查看自己笔记的人一起工作。
...
Token 使用量会因任务而异。短聊天可能使用差不多数量的 token，也可能稍多一些，因为 LCM 会保持最近上下文的新鲜度，并可能在后台做记忆维护。较长的会话通常应该减少反复把旧聊天历史带进主 prompt 所消耗的 token，但节省的一部分可能会用于记忆维护或查找调用。实际使用中，可以预期 LCM 首先优化的是更安全地记住更多内容，其次才是减少长上下文 token 的浪费。

这仍然是预发布版。测试 LCM 时，请使用此仓库的 GitHub Release 产物。普通的 Kilo Code Marketplace 和 npm 包仍然是上游版本。

LCM 的灵感来自论文 [LCM: Lossless Context Management](https://arxiv.org/abs/2605.04050)。更多实现细节可见 [当前代码规范](specifications/README.md)。
