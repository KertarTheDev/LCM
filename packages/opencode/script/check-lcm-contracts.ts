...
if (!kiloRegistry.includes("lcm?: readonly Tool.Info[]") || !kiloRegistry.includes("LcmToolRegistry.build(tools.lcm ?? [])")) {
  fail("the additive LCM tool collection must stay optional with an empty default for upstream callers")
}
