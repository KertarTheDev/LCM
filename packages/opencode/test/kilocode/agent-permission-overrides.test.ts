...
test("shared boards stay available to ordinary agents but cannot reopen hidden recovery permissions", async () => {
  for (const enabled of [false, true]) {
    const config: Partial<ConfigV1.Info> = {
      experimental: { shared_agent_board: enabled },
      permission: { board_read: "allow", board_post: "allow" },
      agent: {
        "lcm-recovery": { permission: { "*": "allow", board_read: "allow", board_post: "allow" } },
        [LCM_RECOVERY_FINALIZER_AGENT]: {
          permission: { "*": "allow", board_read: "allow", board_post: "allow" },
        },
      },
    }
    for (const name of ["lcm-recovery", LCM_RECOVERY_FINALIZER_AGENT]) {
      const agent = await get(config, name)
      expect(agent).toBeDefined()
      for (const tool of ["board_read", "board_post"])
        expect(Permission.evaluate(tool, "*", agent!.permission).action).toBe("deny")
    }
    if (enabled) {
      const ask = await get(config, "ask")
      for (const tool of ["board_read", "board_post", "lcm_query"])
        expect(Permission.evaluate(tool, "*", ask!.permission).action).toBe("allow")
    }
  }
})
