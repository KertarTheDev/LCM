...
describe("LCM isolated recovery contract", () => {
  ...
  test("uses a decoded structured-output format at the internal prompt boundary", () => {
    ...
    expect(JSON.stringify(format.schema)).toContain("candidateEvidence intervals and a raw-source lcm_grep occurrence.citation")
  ...
  test("stops consecutive schema-invalid private calls without treating operational failures as malformed input", () => {
    ...
    expect(
      KiloSessionProcessor.invalidToolInput(new InvalidArgumentsError({ tool: "lcm_read", detail: "expected an object" })),
    ).toBe(true)
  ...
  test("enforces host coverage gaps after incomplete recovery evidence", () => {
    ...
    expect(
      recoveryFullCoverageGaps(resolvedHierarchy, { semanticUnitIndexes: [1, 3], structuralScopeIncomplete: false }),
    ).toEqual(["Exact structural unit 1 retained partial or conflicting semantic coverage."])
    expect(
      recoveryFullCoverageGaps(resolvedHierarchy, { semanticUnitIndexes: [3], structuralScopeIncomplete: true }),
    ).toEqual(["The host-matched exact structural scope omitted one or more requested units."])
