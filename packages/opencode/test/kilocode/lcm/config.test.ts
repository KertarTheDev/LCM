...
describe("LCM configuration", () => {
  test("keeps shared boards and Conversation Memory independently configurable", () => {
    for (const shared_agent_board of [true, false]) {
      for (const conversation_memory of [true, false]) {
        const config = decode({
          experimental: { shared_agent_board, conversation_memory },
          conversation_memory: { recovery: { max_research_steps: 12 } },
        })
        expect(config.experimental?.shared_agent_board).toBe(shared_agent_board)
        expect(enabled(config)).toBe(conversation_memory)
        expect(lcmRecoveryLimits(config).researchMaxSteps).toBe(12)
      }
    }
  })
