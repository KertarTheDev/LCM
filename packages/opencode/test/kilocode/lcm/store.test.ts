...
describe("LCM SQLite store", () => {
  ...
  test("activates the last committed frontier despite equal or regressing wall timestamps", async () => {
    const store = SqliteConversationMemoryStore.open({ databasePath: ":memory:" })
    try {
      const sources = [makeSource(0)]
      const digest = lineageDigest(sources)
      ...
      for (const [id, createdAt] of [
        ...
        expect((await store.activeRevision("ses_test", digest))?.id).toBe(id)
      }
    } finally {
      store.close()
    }
