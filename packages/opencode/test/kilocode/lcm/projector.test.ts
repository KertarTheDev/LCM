...
describe("LCM projector", () => {
  ...
  test("replaces only the eligible prefix and pins a continuation revision", async () => {
    ...
    expect(memory).toContain("use one focused lcm_expand_query for semantic interpretation")
    expect(memory).toContain("A grep hit is a")
    expect(memory).toContain("miss excludes only that spelling")
