...
test("uses upstream lifecycle classification to exclude transient UI progress from memory", () => {
  const input = messages()
  const message = input.find((item) => item.info.role === "assistant")!
  const part = message.parts.find((item) => item.type === "text")!
  if (part.type !== "text") throw new Error("missing assistant text")
  const before = extractFinalSources(sessionID, input)
  part.metadata = { "kilocode.lifecycle": "transient" }
  const after = extractFinalSources(sessionID, input)
  expect(after).toEqual(before.filter((source) => source.metadata.partID !== part.id))
  expect(after.some((source) => source.content.includes(part.text))).toBe(false)
})
