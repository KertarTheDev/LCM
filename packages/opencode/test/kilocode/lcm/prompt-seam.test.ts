...
describe("LCM prompt seam", () => {
  ...
  test("records successful consumption before a superseded queued handoff", async () => {
    const source = await Bun.file(promptPath).text()
    const completed = source.indexOf("yield* completeMemory(true)")
  ...
  test("uses blocking LCM hard maintenance without a legacy compaction fallback", async () => {
    ...
    expect(source).toContain('if (result === "legacy_compact")')
    expect(source).toContain("if (lcmEnabled)")
  })

  test("separates legacy thresholds from provider overflow and verifies the stricter retry", async () => {
    const source = await Bun.file(promptPath).text()
    expect(source).toContain('overflowPolicy: lcmEnabled ? "lcm" : "legacy"')
