...
describe("LCM prompt seam", () => {
  ...
  test("records direct hard-pressure preparation in the activity timeline", async () => {
    ...
    const capacityCheck = source.indexOf('input.measure(result.type === "projected"', activity)

    expect(hard).toBeGreaterThan(0)
    expect(build).toBeGreaterThan(hard)
    expect(activity).toBeGreaterThan(build)
    expect(capacityCheck).toBeGreaterThan(activity)
    expect(source.slice(activity, capacityCheck)).toContain('kind: changed ? "frontier_advanced" : "intervention"')
    expect(source.slice(activity, capacityCheck)).toContain("hard-level preparation")
    expect(source.slice(activity, capacityCheck)).toContain("events.publish(LcmEvent.Activity")
  })
})
