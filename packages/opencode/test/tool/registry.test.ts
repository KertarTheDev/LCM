...
const withBoard = testEffect(
  registryLayer({ config: { get: () => Effect.succeed({ shared_agent_board: true }) } }),
)
// kilocode_change end
    ...
    Effect.gen(function* () {
      ...
      for (const name of ["ask", LCM_RECOVERY_AGENT, LCM_RECOVERY_FINALIZER_AGENT]) {
        ...
        if (name === "ask") {
          ...
          expect(visible).not.toContain("goal_report")
          expect(visible).not.toContain("schedule_wakeup")
          expect(visible).not.toContain("cancel_wakeup")
