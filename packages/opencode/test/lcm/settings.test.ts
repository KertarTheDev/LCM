...
test("session settings route rejects a wrong project assertion before writing config", async () => {
  ...
  try {
    const response = await app.request(`/session/${session.id}/lcm/settings`, {
      ...
      body: JSON.stringify({ projectID: `${session.projectID}-forged`, strategy: "dolt" }),
    })
