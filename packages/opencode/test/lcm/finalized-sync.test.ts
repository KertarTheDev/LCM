...
import type { ModelID, ProviderID } from "../../src/session/lcm/provider-ids"
...
test("sync keeps persisted SWE-pruned tool output canonical when a truncation sidecar validates", async () => {
  await using tmp = await tmpdir({ git: true })
  const dataDir = path.join(tmp.path, "lcm")
  ...
  try {
    ...
    expect(valid?.content_storage_kind).toBe("inline")
    expect(valid?.tool_output_text).toBe("VISIBLE_VALID_TRUNCATED_WRAPPER")
    expect(valid?.content_file_id).toBeNull()
    expect(valid?.artifact_path).toBeNull()
    ...
    expect(valid?.tool_output_text).not.toContain("LCM_VALID_SIDECAR_START")
    expect(valid?.tool_output_text).not.toContain("LCM_VALID_SIDECAR_END")
  } finally {
