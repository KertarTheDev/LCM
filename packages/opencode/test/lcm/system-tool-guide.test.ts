...
test("LCM system tool guide names retrieval, direct-content, and map tool usage", () => {
  ...
  expect(guide).toContain("Literal mode is the default")
  expect(guide).toContain("scopeWarning")
  ...
  expect(guide).toContain("noAnswerReason")
  ...
  expect(guide).toContain("retryAfterMs")
