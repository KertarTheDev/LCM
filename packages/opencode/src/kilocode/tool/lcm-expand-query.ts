...
const STRUCTURAL_UNIT_CARDINALS = new Map<string, number>([
  ["one", 1],
  ["two", 2],
  ["three", 3],
  ["four", 4],
...
const STRUCTURAL_LABEL_FILLERS = new Set(["a", "an", "of", "the"])
const STRUCTURAL_SCOPE_PREPOSITIONS = new Set(["during", "for", "from", "in", "inside", "of", "within"])
const STRUCTURAL_AGGREGATE_WORDS = new Set(["across", "both", "compare", "comparison", "versus", "vs"])
const STRUCTURAL_JOIN_WORDS = new Set(["and", "or", "through", "to", "versus", "vs"])

interface StructuralQueryWord {
  value: string
  start: number
  end: number
}

interface StructuralOrdinalCandidate {
  ordinal: number | "last"
  startWord: number
  endWord: number
  scoped: boolean
...
function structuralQueryWords(query: string) {
  const normalized = query.normalize("NFKC").toLocaleLowerCase()
  return {
    normalized,
    words: [...normalized.matchAll(/[\p{L}\p{N}]+/gu)].flatMap((match): StructuralQueryWord[] =>
      match.index === undefined ? [] : [{ value: match[0], start: match.index, end: match.index + match[0].length }],
    ),
  }
}

function parseStructuralOrdinal(value: string | undefined, allowCardinal: boolean) {
  if (!value) return
  const named = STRUCTURAL_UNIT_ORDINALS.get(value)
  if (named !== undefined) return named
  if (allowCardinal) {
    const cardinal = STRUCTURAL_UNIT_CARDINALS.get(value)
    if (cardinal !== undefined) return cardinal
  }
  if (!allowCardinal && !/^\d{1,3}(?:st|nd|rd|th)$/u.test(value)) return
  if (allowCardinal && !/^\d{1,3}(?:st|nd|rd|th)?$/u.test(value)) return
  const numeric = Number.parseInt(value, 10)
  return Number.isSafeInteger(numeric) && numeric > 0 ? numeric : undefined
}

function structuralBoundaryLabelWords(openingMarker: string) {
  const key = structuralBoundaryIdentity(openingMarker)?.key
  if (!key || key === "unlabelled-unit") return []
  const words = key.match(/[\p{L}\p{N}]+/gu) ?? []
  while (words[0] && STRUCTURAL_LABEL_FILLERS.has(words[0])) words.shift()
  return words
}

function isScopedStructuralCandidate(words: readonly StructuralQueryWord[], startWord: number) {
  return words
    .slice(Math.max(0, startWord - 4), startWord)
    .some((word) => STRUCTURAL_SCOPE_PREPOSITIONS.has(word.value))
}

function naturalStructuralOrdinalCandidates(query: string, openingMarker: string) {
  const labelWords = structuralBoundaryLabelWords(openingMarker)
  if (labelWords.length === 0) return { normalized: "", words: [], candidates: [] }
  const { normalized, words } = structuralQueryWords(query)
  const candidates: StructuralOrdinalCandidate[] = []
  for (let index = 0; index <= words.length - labelWords.length; index++) {
    if (!labelWords.every((word, offset) => words[index + offset]?.value === word)) continue
    const before = parseStructuralOrdinal(words[index - 1]?.value, false)
    const after = parseStructuralOrdinal(words[index + labelWords.length]?.value, true)
    if (before !== undefined) {
      candidates.push({
        ordinal: before,
        startWord: index - 1,
        endWord: index + labelWords.length - 1,
        scoped: isScopedStructuralCandidate(words, index - 1),
      })
    }
    if (after !== undefined) {
      candidates.push({
        ordinal: after,
        startWord: index,
        endWord: index + labelWords.length,
        scoped: isScopedStructuralCandidate(words, index),
      })
    }
  }
  return { normalized, words, candidates }
}

function hasJoinedStructuralOrdinal(
  normalized: string,
  words: readonly StructuralQueryWord[],
  candidate: StructuralOrdinalCandidate,
) {
  const before = words[candidate.startWord - 1]
  const beforeOrdinal = words[candidate.startWord - 2]
  if (before && STRUCTURAL_JOIN_WORDS.has(before.value) && parseStructuralOrdinal(beforeOrdinal?.value, true))
    return true
  const after = words[candidate.endWord + 1]
  const afterOrdinal = words[candidate.endWord + 2]
  if (after && STRUCTURAL_JOIN_WORDS.has(after.value) && parseStructuralOrdinal(afterOrdinal?.value, true)) return true

  for (const adjacentIndex of [candidate.startWord - 1, candidate.endWord + 1]) {
    const adjacent = words[adjacentIndex]
    if (!adjacent || parseStructuralOrdinal(adjacent.value, true) === undefined) continue
    const left = adjacentIndex < candidate.startWord ? adjacent : words[candidate.endWord]!
    const right = adjacentIndex < candidate.startWord ? words[candidate.startWord]! : adjacent
    if (/^[\s,\-/–—]*$/u.test(normalized.slice(left.end, right.start))) return true
  }
  return false
}

function naturalStructuralUnitOrdinal(query: string, openingMarker: string) {
  const { normalized, words, candidates } = naturalStructuralOrdinalCandidates(query, openingMarker)
  if (candidates.length === 0) return
  const unique = [...new Set(candidates.map(({ ordinal }) => ordinal))]
  if (unique.length === 1) {
    if (candidates.some((candidate) => hasJoinedStructuralOrdinal(normalized, words, candidate))) return
    return unique[0]
  }

  const aggregate = words.some((word) => STRUCTURAL_AGGREGATE_WORDS.has(word.value))
  const joined = candidates.some((candidate) => hasJoinedStructuralOrdinal(normalized, words, candidate))
  if (aggregate || joined) return
  const scoped = [...new Set(candidates.filter((candidate) => candidate.scoped).map(({ ordinal }) => ordinal))]
...
function structuralUnitOrdinal(query: string, openingMarker: string) {
  ...
  if (markerAt < 0) return naturalStructuralUnitOrdinal(query, openingMarker)
  ...
  if (!match || match.index === undefined) return naturalStructuralUnitOrdinal(query, openingMarker)
...
export function structuralRecoveryScope(
  ...
  const normalizedQuery = query.normalize("NFKC").toLocaleLowerCase()
  const labelQuery = ` ${normalizedQuery.replace(/[^\p{L}\p{N}]+/gu, " ").trim()} `
  ...
  const scored = keyed.map(({ unit, key }) => {
    ...
    const exactKey = key.length >= 2 && key !== "unlabelled-unit" && labelQuery.includes(` ${key} `)
    const score =
...
function resolvedScopeCoversExactUnit(scope: readonly ResolvedSourceRange[], unit: readonly ResolvedSourceRange[]) {
  return unit.every((range) =>
    scope.some(
      (candidate) =>
        range.sourceID === candidate.sourceID &&
        range.ordinal === candidate.ordinal &&
...
export function trustedStructuralSemanticUnit(input: {
  ...
  for (const unit of structural.index.units) {
    ...
    try {
      ...
      const singleUnitCoveringScopeMatches =
        structural.index.matchedUnits === 1 && resolvedScopeCoversExactUnit(input.sourceRanges, unitRanges)
      if (contentScopeMatches || singleUnitEnvelopeMatches || singleUnitCoveringScopeMatches)
        return {
          ...unit,
...
export function trustedStructuralShardQuestion(input: {
  ...
  return [
    ...
    "Local collection assignment: apply the original event criteria, but defer the requested unit-level ordinal to the reducer. Collect qualifying predecessors as well as possible targets. If fewer than the requested number occur here, return those local events with partial coverage. Asking who acted does not restrict which actors qualify unless the question separately names an actor constraint.",
    rank === 1
...
export function trustedStructuralReductionQuestion(input: {
  ...
  return [
    ...
    rank === 1
      ? `Trusted hierarchical reduction: the supplied evidence contains bounded byte-verified but semantically untrusted local proposals from each of ${input.shardCount} chronological shards of this unit, in shard order. Reapply every original subject, actor, action, scope, and event-status criterion to each exact quote, discard every nonqualifying proposal, then resolve exactly one unit-level value by choosing the ${input.direction === "first" ? "earliest" : "latest"} remaining event. Return that event in the required events array. Do not concatenate shard candidates in answer. A nonqualifying proposal must not suppress a qualifying event.`
      : `Trusted hierarchical reduction: the supplied evidence contains up to ${rank} bounded local events from each of ${input.shardCount} chronological shards of this unit, in shard order. Preserve each local list's declared order and resolve exactly the ${rank}${rank === 2 ? "nd" : rank === 3 ? "rd" : "th"} qualifying event from the ${input.direction === "first" ? "opening" : "closing"} edge, scanning shard objects from that edge as well. Return the ordered ${input.direction === "first" ? "prefix" : "suffix"} through that event in the required events array.`,
    "Event status comes from the authoritative question: plans, mentions, rejected attempts, hypotheticals, and continuations qualify when requested. They do not prove a newly completed action. Do not impose a completed-action filter on other questions.",
  ].join("\n")
}
...
export function trustedStructuralPassRepairQuestion(input: {
  ...
  eventRequirement?: { rank: number; verified: number }
}) {
  return [
    input.question,
    ...
    ...(input.eventRequirement
      ? [
          `The requested ordinal requires ${input.eventRequirement.rank} distinct qualifying events from the requested edge through the target; the host verified ${input.eventRequirement.verified} event records in the preceding attempt. Return that entire prefix or suffix in events, with a separate exact qualifying-event quote for each value. A target-only record cannot establish its ordinal. Do not invent events to meet the count; retain uncertainty if the evidence cannot establish it.`,
        ]
      : []),
  ].join("\n")
}
...
export function hierarchicalSemanticShardRepairPlan(input: {
  ...
  // Invalid shards have no usable proposal set. Subdivide them before spending calls on other uncertainty;
  // a same-size retry is only a fallback when the remaining allowance cannot fund smaller scans.
  for (const index of order) {
    if (input.shards[index]?.parsed || remaining < 1) continue
    const parts = remaining >= repairParts ? repairParts : 1
    result.push({ index, reason: "invalid_response", parts })
    remaining -= parts
  }
...
export function structuralPassEventRejection(
  answer: QueryAnswer | undefined,
  selected: Parameters<typeof hostVerifiedSemanticEvents>[1],
  rank: number,
): QueryResponseRejection | undefined {
  if (!answer || answer.coverage === "none") return
  if (!answer.events?.length) return "missing_structured_events"
  const verified = hostVerifiedSemanticEvents(answer, selected).length
  if (verified === 0) return "unverified_structured_events"
  if (verified < Math.max(1, Math.min(QUERY_EVIDENCE_PROPOSAL_LIMIT, Math.floor(rank))))
    return "insufficient_verified_events"
}
...
export function prefetchedIsolatedQueryEvidence(input: {
  ...
  const coverageExpectation = {
    semanticUnitIndexes:
      retrievalTruncated && boundedStructural
        ? boundedStructural.index.units.flatMap((unit) => (unit.contentScope ? [unit.index] : []))
        : [],
    // A missing combined envelope is a transport limit, not missing units: each mapped unit can still be resolved.
    structuralScopeIncomplete: Boolean(structural && (!boundedStructural || boundedStructural.index.truncated)),
  }
        ...
        Effect.gen(function* () {
          ...
          if (
            ...
            const analyzeSemanticPass = (
              ...
              const rejection =
                parsedResult?.rejection ??
                structuralPassEventRejection(parsedResult?.answer, semanticCandidates, semanticRank)
                  ...
                  reductionAttempts.push(
                    yield* runSemanticPass(
                      trustedStructuralPassRepairQuestion({
                        ...
                        eventRequirement: {
                          rank: semanticRank,
                          verified: hostVerifiedSemanticEvents(failed.candidate, semanticCandidates).length,
                        },
                      }),
                      reductionEvidence,
