...
import { extractFinalSources, normalizeTranscriptChronology } from "@/kilocode/session/lcm/transcript-source"
...
export const loadMemory = Effect.fn("LcmTool.loadMemory")(function* (input: {
  ...
  return {
    ...
    transcript: normalizeTranscriptChronology(transcript),
  } satisfies MemoryView
})

export function lcmMemorySessionID(ctx: Pick<Tool.Context, "sessionID" | "extra">) {
  const source = ctx.extra?.lcmSourceSessionID
  return typeof source === "string" ? SessionID.make(source) : ctx.sessionID
}

export function isolatedRecoveryPriorTurnCutoff(
  ctx: Pick<Tool.Context, "extra">,
  view: Pick<MemoryView, "sources" | "transcript">,
) {
  if (typeof ctx.extra?.lcmSourceSessionID !== "string") return
  return priorTurnSourceCutoff(view, view.transcript) ?? -1
}

export function requireIsolatedRecoverySource(
  ctx: Pick<Tool.Context, "extra">,
  view: Pick<MemoryView, "sources" | "transcript">,
  source: Pick<FinalSource, "ordinal">,
) {
  const cutoff = isolatedRecoveryPriorTurnCutoff(ctx, view)
  if (cutoff !== undefined && source.ordinal > cutoff)
    throw new LcmToolError("lcm_not_found", "The source is not part of earlier parent-session memory.")
}

export function requireIsolatedRecoverySummary(
  ctx: Pick<Tool.Context, "extra">,
  view: Pick<MemoryView, "sources" | "transcript">,
  summary: Pick<SummaryNode, "lastOrdinal">,
) {
  const cutoff = isolatedRecoveryPriorTurnCutoff(ctx, view)
  if (cutoff !== undefined && summary.lastOrdinal > cutoff)
    throw new LcmToolError("lcm_not_found", "The summary is not wholly inside earlier parent-session memory.")
}
...
type RecoveryBatchState = {
  calls: number
  signatures: Map<string, number>
}

// Every tool in one provider response receives the same frozen messages array. Historical transcript inspection cannot
// see sibling calls from that response, so reserve their canonical inputs synchronously before execution continues.
// A WeakMap keeps the coordination scoped to that response without retaining completed messages or session state.
const recoveryBatchStates = new WeakMap<readonly unknown[], RecoveryBatchState>()
...
export function reserveRecoveryBatchCall(messages: readonly unknown[], tool: RecoveryTool, input: unknown) {
  let state = recoveryBatchStates.get(messages)
  if (!state) {
    state = { calls: 0, signatures: new Map() }
    recoveryBatchStates.set(messages, state)
  }
  const signature = `${tool}:${JSON.stringify(canonicalRecoveryToolInput(tool, input))}`
  const previousIdenticalCalls = state.signatures.get(signature) ?? 0
  state.calls++
  state.signatures.set(signature, previousIdenticalCalls + 1)
  return { batchCallNumber: state.calls, previousIdenticalCalls }
}
...
export function sourceChronology(
  ...
  const ordered = [...view.sources.values()]
    .filter((source) => maximumOrdinal === undefined || source.ordinal <= maximumOrdinal)
