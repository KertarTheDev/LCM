...
export const LCM_QUERY_QUESTION_DESCRIPTION =
  "One concise, focused question about earlier current-session conversation memory. Narrow the scope without changing the user's meaning: preserve the user's verb, qualifiers, inclusion and exclusion rules, event definition, and evidence standard. Do not add stricter terms such as explicit or exact unless the user requested them. Retain the user's exactness, completeness, ordering, and boundary requirements; do not introduce new ones. The host rejects detectable initial or follow-up criteria changes without spending a child allowance. Pass only the question, not evidence, examples, citations, or raw history. A result-informed follow-up must identify a distinctive term from the preceding answer or named unresolved gap/boundary so the host can verify that it is narrower. Treat partial answers and other candidates as hypotheses to check, not facts to introduce through a new if, assuming, given-that, or provided-that premise."

export const LCM_QUERY_DESCRIPTION =
  "Ask one concise, focused question about an earlier current-session detail that is missing or uncertain after using the relevant facts already visible in active context. Narrow scope without changing the user's meaning: preserve the user's verb, qualifiers, inclusion and exclusion rules, event definition, and evidence standard; do not add stricter terms such as explicit or exact unless the user requested them. Pass only the question, not evidence, examples, citations, or raw history. The host privately prefetches bounded evidence and, when paired boundaries match the question, gives their exact raw structural scope only to a hidden read-only recovery agent. That agent may navigate or verify exact details and reason in a separate child context. The main session receives only a short direct answer, coverage, unresolved gaps, and at most six host-verified exact source excerpts of at most 512 bytes each. Combine that bounded result with the active context; partial or empty recovery does not erase independently supported visible facts. Retain the user's first/last, Nth, count, exhaustive-list, ordering, and semantic-boundary requirements; do not introduce new ones. Prefer one complete question; ask a narrower follow-up only after the first bounded result reports partial or no coverage and names a decisive unresolved gap. The follow-up question must identify a distinctive term from the preceding answer or named unresolved gap/boundary so the host can verify that it is result-informed rather than a broad restart. Treat partial answers and other candidates as hypotheses to check, not facts to introduce through a new if, assuming, given-that, or provided-that premise. For an exact, count, exhaustive-list, first/last/Nth, or ordering request, partial coverage cannot establish the answer unless active context independently closes every named gap; otherwise use the narrower follow-up before finalizing. When such a completeness-sensitive question remains partial, the host withholds the polished candidate answer so it cannot be mistaken for a complete result; bounded citations and named gaps remain available. The host starts no parallel second child and gives an admitted follow-up child the preceding bounded result as provisional evidence."
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const parentVisible = protectIncompleteRecoveryAnswer(
            completed.verified,
            [parentContext, question].filter(Boolean).join("\n"),
          )
