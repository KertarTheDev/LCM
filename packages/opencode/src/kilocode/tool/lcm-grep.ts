...
import { LCM_RECOVERY_CITATION_BYTES } from "@/kilocode/session/lcm/recovery-contract"
import {
...
import { validUtf8Offset } from "./lcm-read"
...
export function occurrencePaginationAdvice(
  sourceScoped: boolean,
  matchCounts: number[],
  occurrenceOffset = 0,
) {
  if (!sourceScoped) return
  const matchCount = Math.max(0, ...matchCounts)
  if (occurrenceOffset > 0 && matchCount > 0 && occurrenceOffset >= matchCount)
    return `This occurrence page started at zero-based match ${occurrenceOffset}, beyond the ${matchCount} matches in the searched scope. The pattern is present; an empty page does not mean it is absent. Retry with occurrenceOffset 0, or use occurrencePage.lastOffset exactly as returned.`
  if (occurrenceOffset > 0)
    return `This occurrence page intentionally omitted the first ${occurrenceOffset} matches. Do not treat the returned page as the first match or as complete evidence before that offset.`
  if (!matchCounts.some((count) => count > MAX_RANGES_PER_RECORD)) return
...
export function hasExactMatches(result: { matchCount: number }) {
...
export const LcmGrepParameters = Schema.Struct({
  ...
  occurrenceOffset: Schema.optional(Schema.Number).annotate({
    description:
      "Zero-based match index for paging a source-scoped search in groups of 20; it is not a byte offset. Omit it on the first call, then copy nextOffset or lastOffset exactly from the preceding occurrencePage.",
  }),
...
export function grepOccurrence(input: {
  fullText: string
  ...
  const total = Buffer.byteLength(input.fullText)
  const scopeStart = Math.max(0, input.scopeStart ?? 0)
  const scopeEnd = Math.min(total, input.scopeEnd ?? total)
  const matchBytes = input.byteRange.end - input.byteRange.start
  const spare = Math.max(0, LCM_RECOVERY_CITATION_BYTES - matchBytes)
  let startOffset = Math.max(scopeStart, input.byteRange.start - Math.floor(spare * 0.4))
  let endOffset = Math.min(scopeEnd, startOffset + LCM_RECOVERY_CITATION_BYTES)
  if (endOffset < input.byteRange.end) {
    endOffset = input.byteRange.end
    startOffset = Math.max(scopeStart, endOffset - LCM_RECOVERY_CITATION_BYTES)
  }
  while (startOffset < input.byteRange.start && !validUtf8Offset(input.fullText, startOffset)) startOffset++
  while (endOffset > input.byteRange.end && !validUtf8Offset(input.fullText, endOffset)) endOffset--
  const excerpt = Buffer.from(input.fullText).subarray(startOffset, endOffset).toString("utf8")
  const excerptByteRange = { start: startOffset, end: endOffset }
  return {
    range: input.range,
    byteRange: input.byteRange,
    excerpt,
    excerptByteRange,
    ...(input.sourceID && excerptByteRange.start <= input.byteRange.start && excerptByteRange.end >= input.byteRange.end
      ? { citation: { sourceID: input.sourceID, startOffset, endOffset } }
      : {}),
  }
}
        ...
        Effect.gen(function* () {
          ...
          const found =
            (params.mode ?? "literal") === "regex"
              ...
              : values
                  ...
                  // Retain an exact positive count even when the requested occurrence page begins beyond its end.
                  // Otherwise an empty page is indistinguishable from lexical absence.
                  .filter(hasExactMatches)
          ...
          const matches = selected.map((item) => {
            ...
            const occurrences = ranges.map((range, index) => {
              const occurrence = grepOccurrence({
                fullText: value.fullText,
                range,
                byteRange: byteRanges[index]!,
                ...(value.kind === "source" ? { sourceID: value.publicID } : {}),
                scopeStart: value.byteOffset,
                scopeEnd: value.endOffset,
              })
              return occurrence
            })
            return {
              ...
              occurrencePage: {
                ...
                hasEarlierOccurrences: occurrenceOffset > 0 && item.matchCount > 0,
                ...(params.sourceID && (item.matchCount > MAX_RANGES_PER_RECORD || occurrenceOffset > 0)
          ...
          const advice = [
            lexicalSearchAdvice(matches.reduce((total, match) => total + match.matchCount, 0)),
