...
export function structuralBoundaryIdentity(marker: string) {
...
export function pairedStructuralUnits(structural: StructuralAnchorIndex) {
  ...
  for (const anchor of structural.anchors) {
    const boundary = structuralBoundaryIdentity(anchor.marker)
...
function render(items: MemoryItem[], structural: StructuralAnchorIndex) {
  ...
  for (const unit of paired.units) {
    const line = `- ${unit.opening.marker} → ${unit.closing.marker}: sources ${unit.opening.sourceID} through ${unit.closing.sourceID}`
  ...
  const structuralMap =
    structural.total === 0
      ...
      : [
          ...
          "questions. Byte intervals are half-open. Include the relevant boundary label or stable handle in one focused",
          ...
          ...(pairedLines.length > 0
            ? [
                "",
                "Paired semantic-unit labels for focused recovery:",
                "Name the relevant unit in one lcm_query question; the isolated researcher verifies its exact ranges.",
                ...pairedLines,
                ...(pairedLines.length < paired.total
                  ? [
                      `[Paired semantic-unit map incomplete: showing ${pairedLines.length} of ${paired.total}; lcm_query can recover omitted or larger units privately.]`,
                    ]
                  : []),
              ]
            : []),
  ...
  return [
    ...
    "boundary-sensitive, first/last, count, or complete-list questions, ask one focused lcm_query. Pass only the",
    "question; do not copy candidate answers, examples, evidence, or raw history into its argument. State the needed",
    "scope and completeness explicitly. Its hidden read-only child agent performs semantic search, exact grep/read",
    "verification, and multi-step reasoning privately. The main session receives only a concise answer, coverage,",
    "unresolved gaps, and a bounded set of host-verified exact source excerpts. Do not reconstruct, page, or manually",
    "aggregate raw history in the main context. If coverage is partial, ask a narrower follow-up only when its named gap",
    "blocks the user answer. Once the isolated result resolves the question, answer immediately.",
    ...structuralMap,
    "",
    body,
    MEMORY_CLOSE,
...
export class Projector {
  ...
  async project(input: ProjectionInput): Promise<ProjectionResult> {
    ...
    if (result.type === "projected" && result.pressureAfter >= 1)
      return { type: "unchanged", messages: input.messages, pressure: result.pressureBefore }
    return result
  }

  /** Internal candidate for hard-maintenance measurement; callers must check capacity before provider dispatch. */
  async assess(input: ProjectionInput): Promise<ProjectionResult> {
    ...
    if (activeTokens >= rawTokens) return { type: "unchanged", messages: input.messages, pressure }
