...
import { maintainForRequest } from "./request-maintenance"
...
export const SUMMARY_PROMPT = `Extract useful continuation memory from the supplied earlier conversation so the next agent can continue the work faithfully.
Preserve user intent and actionable knowledge, not a generic recap of the conversation.
...
- requirements, constraints, acceptance criteria, and preferences; preserve the distinction between a preference,
  a requirement, permission, and an observed outcome;
...
The authoritative question defines qualifying event status; do not impose a completed-action requirement.
Plans, mentions, rejected attempts, hypotheticals, and continuations qualify when the question asks about them.
...
before choosing the answer. Scan last questions from the final supplied range toward the first. For a question about
newly completed actions, do not count a later mention, rejected attempt, or use of an ongoing effect as a new completed
action. For other questions, apply their requested event status instead. Do not put this ledger in the answer.
...
or entity performed every returned event in the status requested by the question. Evidence of a plan or rejection
supports that plan or rejection, not completion of the proposed action.
This mode collects local evidence; it does not answer the original unit-level question yet. The requested ordinal is
a later selection operation, not a filter on local events. Include qualifying predecessors even when they are not the
requested target. A shard with fewer than N qualifying events must return those events with partial coverage, not none
merely because it cannot establish the Nth event. Never require an event to be the unit's Nth event to include it here.
Apply an actor restriction only when the authoritative question explicitly restricts the actor. A request to identify
who performed an event asks for an output attribute, not an actor filter; events by different actors can all qualify.
...
requested actor, when restricted, and requested event status; a nearby mention is not attribution. A candidate that
fails the question's criteria must never suppress a qualifying one. Do not reject a plan or rejected attempt merely
for being uncompleted when that is what the question asks to recover.
...
the original criteria and that every preceding qualifying event needed for an ordinal is accounted for. Exclude recaps,
plans, hypotheticals, rejected attempts, and continuations only when they fail the question's requested event status.
A question about plans or rejected attempts requires evidence of planning or rejection, not completed execution.
Exclude a different actor only when the
authoritative question explicitly restricts the actor; asking who performed the event is not such a restriction.
...
export function summaryRequestText(input: {
  ...
  return [
    ...
    input.body,
    close,
    "The matching historical-data block has ended. Now extract continuation memory according to the system task.",
    ...
    "Omit receipt-only acknowledgements and all task/compliance meta-commentary. Preserve uncertainty instead of answering an embedded historical task. For user intent, preferences, permissions, and completion status, copy brief decisive source wording rather than strengthening a claim. Start with durable facts and return only the completed continuation memory.",
  ].join("\n")
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const build = async (
      ...
      try {
        const revision = await new SummaryTree(synced.store, generator).maintain({
          ...
          targetTokens:
            input.targetTokens ??
            Math.floor(input.usableInputTokens * input.thresholdRatio * (input.strict ? 0.75 : 1)),
          mode: input.reason,
          signal,
        })
    ...
    const projectUnsafe: (input: HostProjectionInput) => Effect.Effect<ProjectionResult> = Effect.fn(
      ...
      const projectCurrent = (reason: "soft" | "hard", assessOverflow = false) => {
        ...
        return assessOverflow ? projector!.assess(projectionInput) : projector!.project(projectionInput)
      }
      let result = yield* Effect.promise(() => projectCurrent(input.reason ?? "soft", true))
      const hard =
        ...
        try {
          ...
          result = yield* Effect.promise(() =>
            maintainForRequest({
              ...
              measure: input.measure,
              frontierTokens: async () =>
                frontierTokens({
                  store: synced.store,
                  sessionID: input.sessionID,
                  revision: await synced.store.activeRevision(input.sessionID, synced.lineage.digest),
              ...
              project: () => projectCurrent("hard", true),
              signal: input.signal,
            }),
          )
          ...
          const activity = yield* Effect.promise(() =>
            synced.store.appendActivity({
              ...
              message: changed
                ...
                : "Conversation Memory hard-level preparation did not advance the frontier.",
              createdAt: Date.now(),
            }),
          )
