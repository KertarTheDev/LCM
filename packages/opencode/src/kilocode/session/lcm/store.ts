...
CREATE INDEX IF NOT EXISTS lcm_frontier_revision_commit_idx
  ON lcm_frontier_revision(session_id, lineage_digest);
...
export class SqliteConversationMemoryStore implements ConversationMemoryStore {
  ...
  async activeRevision(sessionID: string, lineageDigest: string): Promise<FrontierRevision | undefined> {
    // Publication order is authoritative. Wall timestamps can tie or move backwards,
    // and the random suffix of a revision ID does not order same-millisecond commits.
