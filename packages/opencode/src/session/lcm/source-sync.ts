...
async function partWithValidatedTruncationSidecar(input: {
  ...
  // Validate the sidecar as recovery evidence, but keep the finalized persisted
  // ToolPart (including any SWE pruning) as the canonical LCM dialog source.
  await readValidatedTruncationSidecar({
    part: input.part,
    abortSignal: input.abortSignal,
    operationID: input.operationID,
  })
