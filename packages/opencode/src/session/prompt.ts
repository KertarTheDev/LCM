...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const getModel: (
      ...
      modelID: ModelID,
      sessionID: SessionID,
    ) => Effect.Effect<Provider.Model> = Effect.fn("SessionPrompt.getModel")(function* (
      providerID: ProviderID,
      modelID: ModelID,
      sessionID: SessionID,
    ) {
      return yield* provider.getModel(providerID, modelID).pipe(
    ...
    const commandImpl = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
    ...
    const command: Interface["command"] = (input) =>
      commandImpl(input).pipe(
        Effect.catchTag("ProviderModelNotFoundError", (error) => Effect.die(error)), // kilocode_change - model validation reports the user-facing error before this boundary
      )
