...
import {
  ...
  lcmRecoveryInvalidToolInputLimitReached, // kilocode_change
  lcmRecoveryLimits,
  lcmSessionContextManagementEnabled, // kilocode_change
  ...
  Effect.gen(function* () {
    ...
    const runLoop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, NotFoundError> = Effect.fn(
      ...
      while (true) {
        ...
        // kilocode_change start - two consecutive schema-invalid private calls are enough evidence that this research
        // phase cannot safely reconstruct the model's intended arguments; return its exact transcript to the bounded
        // tool-free finalizer instead of spending the remaining configurable steps on the same malformed call
        if (lcmRecoveryInvalidToolInputLimitReached(lastUser.agent, msgs)) {
          yield* Effect.logWarning("isolated recovery invalid tool-input limit reached", {
            "session.id": sessionID,
            agent: lastUser.agent,
          })
          break
        ...
        // kilocode_change end
        // kilocode_change start - recovery children keep their exact short-lived research transcript; their primitives
        // remain bound to the parent source namespace, so projecting a child-owned LCM frontier would be destructive.
        const conversationMemorySessionEnabled = lcmSessionContextManagementEnabled(
        ...
        const outcome: "break" | "continue" = yield* Effect.gen(function* () {
          ...
          if (size > REQUEST_PRUNE_BYTES && (!conversationMemoryEnabled || conversationMemorySessionEnabled)) {
          ...
          } else if (size > REQUEST_PRUNE_BYTES && conversationMemoryEnabled) {
            yield* Effect.logWarning("preserving oversized isolated recovery transcript for provider settlement", {
              "session.id": sessionID,
              size,
            })
          }
          ...
          const transcript = conversationMemorySessionEnabled
          ...
          preparedRequest = yield* ConversationMemoryPromptHost.prepare({
            enabled: conversationMemorySessionEnabled,
          ...
          if (result === "compact") {
            ...
            if (conversationMemoryEnabled) {
              if (!conversationMemorySessionEnabled) {
                handle.message.error = new MessageV2.ContextOverflowError({
                  message:
                    "lcm_recovery_context_limit: The isolated recovery transcript exceeded the selected provider context window.",
                }).toObject()
      ...
      // kilocode_change start - opt-in end-of-turn pruning must preserve private recovery evidence too
      const result = yield* lastAssistant(sessionID)
      if (!ConversationMemoryFeature.enabled(yield* config.get()) || !isLcmRecoveryAgent(result.info.agent)) {
