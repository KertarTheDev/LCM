...
Commands:
  ...
  kilo debug lcm-db-recover-lock  run content-safe LCM DB owner-lock recovery support
...
### kilo debug lcm-db-recover-lock

```
run content-safe LCM DB owner-lock recovery support

Options:
  --help      Show help  [boolean]
  --version   Show version number  [boolean]
  --data-dir  Explicit LCM family root, normally <kilo-data-dir>/lcm/families/<family-id>  [string] [required]
  --dry-run   report the owner-lock recovery action without changing DB files  [boolean]
  --apply     quarantine a recoverable owner.lock and reopen the existing DB  [boolean]
  --force     permit recovery for malformed or otherwise uncheckable owner locks  [boolean] [default: false]
  --json      print JSON report  [boolean] [default: false]
```
