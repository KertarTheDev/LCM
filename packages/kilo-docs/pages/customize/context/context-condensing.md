...
During work, the model uses `lcm_query` to delegate one focused memory question. A locked hidden
recovery worker can privately use:
...
Only the bounded answer, coverage, and optional verified citations return to the main session; raw
worker evidence stays private. These tools and the LCM inspection UI are hidden while the feature is disabled. Existing derived data is preserved
...
Choose the transformation model with **Settings → Models → Compaction model**. The Compaction
section in **Settings → Context** links to that selector. LCM uses this same upstream model
preference; clearing it restores the current session model.
