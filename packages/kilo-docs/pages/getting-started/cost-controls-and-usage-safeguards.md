...
Soft maintenance starts at 40% raw-conversation pressure by default. Complete outgoing requests are also checked
...
`conversation_memory.soft_threshold_percent` in `kilo.jsonc`.
...
- Keep Conversation Memory at its 40% default, or lower `conversation_memory.soft_threshold_percent` to start reducing
