...
describe("Conversation Memory webview state", () => {
  ...
  test("binds timeline and export controls to the displayed local LCM status", async () => {
    ...
    expect(settings).not.toContain("ModelSelectorBase")
    expect(settings).toContain("props.onNavigateToModels?.()")
    expect(models).toContain("config().agent?.compaction?.model")
