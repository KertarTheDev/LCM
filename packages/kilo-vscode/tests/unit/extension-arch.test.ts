...
describe("Extension — LCM prewarm", () => {
  ...
  it("waits for prompt-send LCM readiness with structured safe errors", () => {
    const providerText = fs.readFileSync(KILO_PROVIDER_FILE, "utf-8")
    const prewarmText = fs.readFileSync(LCM_PREWARM_FILE, "utf-8")
    const extensionMessagesText = fs.readFileSync(EXTENSION_MESSAGES_FILE, "utf-8")

    expect(providerText).toContain('this.prewarmLcmSession(sid, "promptSend", dir)')
    expect(providerText).toContain('this.waitForLcmReadyToSend(sid, dir, "promptSend")')
    expect(providerText).toContain("extractLcmSafeError")
    ...
    expect(prewarmText).toContain("waitUntilReady")
