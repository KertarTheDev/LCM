...
import type { ExtensionMessage, LcmActivity, LcmStatus, VSCodeAPI } from "../types/messages"
...
export function createLcmState(input: {
  ...
  const requestStatus = (sessionID: string) => input.vscode.postMessage({ type: "requestLcmStatus", sessionID })
  ...
  createEffect(() => {
    ...
    setError(undefined)
    setActivity([])
    if (active && sessionID && !sessionID.startsWith("cloud:") && connected) requestStatus(sessionID)
  })
