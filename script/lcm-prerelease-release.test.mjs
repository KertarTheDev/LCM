...
test("release notes bind the setup guide, paper, tag, and SHA", () => {
  ...
  assert.match(body, /lcm_grep.*sourceRanges/s)
  assert.match(body, /lcm_expand_query.*retry/s)
  assert.match(body, /coverage.*partial/s)
  assert.match(body, /175k.*pending/s)
})
