...
const tag = "v7.7.3-lcm.1"
const reviewedBody = renderPrereleaseNotes({ base: "7.7.3", tag, sha })
...
test("a free upstream base starts again at lcm.1", () => {
  assert.deepEqual(nextPrereleaseVersion("7.5.15", [], []), {
    base: "7.5.15",
    version: "7.5.15-lcm.1",
    tag: "v7.5.15-lcm.1",
  })
})

test("a published lcm.1 advances to lcm.2", () => {
  assert.deepEqual(nextPrereleaseVersion("7.5.15", [release({ tag_name: "v7.5.15-lcm.1" })], []), {
    base: "7.5.15",
    version: "7.5.15-lcm.2",
    tag: "v7.5.15-lcm.2",
  })
})
...
test("next prerelease suffix counts releases, drafts, and tags", () => {
  const value = nextPrereleaseVersion(
    "7.5.15",
    [release(), release({ tag_name: "v7.5.15-lcm.3", draft: true })],
    [
      { ref: "refs/tags/v7.5.15-lcm.2" },
      { ref: "refs/tags/v7.5.15-lcm.4" },
      { ref: "refs/tags/v7.5.15-lcm.5" },
      { ref: "refs/tags/v7.4.20-lcm.99" },
    ],
  )
  assert.deepEqual(value, { base: "7.5.15", version: "7.5.15-lcm.6", tag: "v7.5.15-lcm.6" })
})

test("nextVersion refuses a second release for one workflow SHA", async () => {
  await assert.rejects(
    nextVersion(options({ command: "next-version", base: "7.5.15" }), {
...
test("release notes bind the setup guide, paper, tag, and SHA", () => {
  const body = renderPrereleaseNotes({ base: "7.7.3", tag, sha })
  ...
  assert.equal(LCM_PREVIOUS_PRERELEASE_TAG, "v7.5.15-lcm.2")
  assert.match(body, /What changed since v7\.5\.15-lcm\.2/)
  assert.match(body, /soft threshold remains 60%/s)
  assert.match(body, /copy-ready citation ranges.*lcm_query worker/s)
  assert.match(body, /schema v14\/tree v11.*SQLite/s)
  assert.match(body, /100k.*512-byte citation/s)
  assert.match(body, /13 of 18 summary attempts hit output limits/)
  assert.match(body, /Continuation-focused summaries/)
  assert.match(body, /Complete-request hard fitting/)
  assert.match(body, /Native Kilo v7.7.3 integration/)
  assert.match(body, /## Evidence and limitations/)
  assert.doesNotThrow(() => validatePrereleaseNotesBody(body.replaceAll("100k", "long-session")))
})
...
test("fork workflow binds release identity and focused v7.7.3 adaptation gates", async () => {
  ...
  for (const command of [
    ...
    "webview-ui/src/context/session-types.ts",
  ...
  assert.equal(fork.includes('"refs/tags/v${base}:refs/tags/v${base}"'), true)
  assert.ok(fork.indexOf('git fetch --no-tags https://github.com/Kilo-Org/kilocode.git') < fork.indexOf('check-opencode-annotations.ts --base "v${base}"'))
...
test("generator exception accepts only the two exact upstream prose omissions", () => {
  const notes = [
  ...
  assert.doesNotThrow(() => validateGeneratedDocs(original, "before\n\nafter\n"))
  assert.doesNotThrow(() => validateGeneratedDocs(original, original))
  assert.throws(() => validateGeneratedDocs(original, "unexpected\n"), /Unexpected generated/)
  assert.throws(() => validateGeneratedDocs(original, original.replace("before", "changed")), /Unexpected generated/)
})
