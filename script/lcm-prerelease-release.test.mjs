...
const tag = "v7.5.15-lcm.1"
const reviewedBody = renderPrereleaseNotes({ base: "7.5.15", tag, sha })
...
test("a published lcm.1 advances to lcm.2", () => {
  assert.deepEqual(nextPrereleaseVersion("7.5.15", [release()], []), {
    base: "7.5.15",
...
test("release notes bind the setup guide, paper, tag, and SHA", () => {
  const body = renderPrereleaseNotes({ base: "7.5.15", tag, sha })
  ...
  assert.equal(LCM_PREVIOUS_PRERELEASE_TAG, null)
  assert.match(body, /What LCM adds to upstream Kilo v7\.5\.15/)
  assert.match(body, /soft_threshold_percent.*remains 60%/s)
  assert.match(body, /lcm_query.*locked hidden child.*host-verified citations/s)
  assert.match(body, /schema v14\/tree v11.*SQLite/s)
  assert.match(body, /175k.*1\.0.*wrong recipient.*partial coverage/s)
  assert.match(body, /do not establish reliable complete recovery or a deterministic score gain/)
  assert.match(body, /excluded from board tools and notifications/)
  assert.match(body, /explicitly enabled end-of-turn pruning/)
  assert.match(body, /Prerequisite lookups and independent questions/)
  assert.match(body, /no longer removes ordinary tools/)
  assert.match(body, /## Evidence and limitations/)
  assert.doesNotThrow(() => validatePrereleaseNotesBody(body.replaceAll("175k", "long-session")))
})
...
test("fork workflow binds release identity and focused v7.5.15 adaptation gates", async () => {
