...
const tag = "v7.5.9-lcm.1"
const reviewedBody = renderPrereleaseNotes({ base: "7.5.9", tag, sha })
...
test("a free upstream base starts again at lcm.1", () => {
  assert.deepEqual(nextPrereleaseVersion("7.5.9", [], []), {
    base: "7.5.9",
    version: "7.5.9-lcm.1",
    tag: "v7.5.9-lcm.1",
  })
...
test("a published lcm.1 advances to lcm.2", () => {
  assert.deepEqual(nextPrereleaseVersion("7.5.9", [release()], []), {
    base: "7.5.9",
    version: "7.5.9-lcm.2",
    tag: "v7.5.9-lcm.2",
  })
...
test("next prerelease suffix counts releases, drafts, and tags", () => {
  const value = nextPrereleaseVersion(
    "7.5.9",
    [release(), release({ tag_name: "v7.5.9-lcm.3", draft: true })],
    [
      { ref: "refs/tags/v7.5.9-lcm.2" },
      { ref: "refs/tags/v7.5.9-lcm.4" },
      { ref: "refs/tags/v7.5.9-lcm.5" },
      { ref: "refs/tags/v7.4.20-lcm.99" },
    ],
  )
  assert.deepEqual(value, { base: "7.5.9", version: "7.5.9-lcm.6", tag: "v7.5.9-lcm.6" })
})

test("nextVersion refuses a second release for one workflow SHA", async () => {
  await assert.rejects(
    nextVersion(options({ command: "next-version", base: "7.5.9" }), {
...
test("release notes bind the setup guide, paper, tag, and SHA", () => {
  const body = renderPrereleaseNotes({ base: "7.5.9", tag, sha })
  ...
  assert.match(body, /What LCM adds to upstream Kilo v7\.5\.9/)
  assert.match(body, /soft_threshold_percent.*40% to 60%/s)
  assert.match(body, /lcm_query.*locked hidden child.*host-verified citations/s)
  assert.match(body, /schema v14\/tree v11.*SQLite/s)
  assert.match(body, /175k.*0\.5.*zero product findings.*do not establish a deterministic score gain/s)
})
...
test("fork workflow binds release identity and focused v7.5.9 adaptation gates", async () => {
