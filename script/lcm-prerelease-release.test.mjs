...
test("a free upstream base starts again at lcm.1", () => {
  assert.deepEqual(nextPrereleaseVersion("7.4.20", [], []), {
    base: "7.4.20",
    version: "7.4.20-lcm.1",
    tag: "v7.4.20-lcm.1",
  })
})

test("a published lcm.1 advances to lcm.2", () => {
  assert.deepEqual(nextPrereleaseVersion("7.4.20", [release()], []), {
    base: "7.4.20",
    version: "7.4.20-lcm.2",
    tag: "v7.4.20-lcm.2",
  })
})
...
test("next prerelease suffix counts releases, drafts, and tags", () => {
  const value = nextPrereleaseVersion(
    "7.4.20",
    [release(), release({ tag_name: "v7.4.20-lcm.3", draft: true })],
    [{ ref: "refs/tags/v7.4.20-lcm.2" }, { ref: "refs/tags/v7.4.19-lcm.99" }],
  )
  assert.deepEqual(value, { base: "7.4.20", version: "7.4.20-lcm.4", tag: "v7.4.20-lcm.4" })
})

test("nextVersion refuses a second release for one workflow SHA", async () => {
  await assert.rejects(
    nextVersion(options({ command: "next-version", base: "7.4.20" }), {
...
test("fork workflow binds release identity and focused v7.4.20 adaptation gates", async () => {
