...
test("release notes bind the setup guide, paper, tag, and SHA", () => {
  ...
  assert.equal(LCM_PREVIOUS_PRERELEASE_TAG, "v7.5.15-lcm.1")
  assert.match(body, /What changed since v7\.5\.15-lcm\.1/)
  assert.match(body, /soft threshold remains 60%/s)
  assert.match(body, /lcm_query.*worker can descend.*relevant branch/s)
  assert.match(body, /schema v14\/tree v11.*SQLite/s)
  assert.match(body, /175k.*0\.75.*partial or withheld/s)
  assert.match(body, /does not establish reliable complete recovery, a deterministic quality gain, or optimal settings/)
  assert.match(body, /Root promotion cannot starve/)
  assert.match(body, /Complete structural navigation survives display limits/)
