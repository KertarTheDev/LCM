...
export const LCM_PREVIOUS_PRERELEASE_TAG = "v7.5.15-lcm.2"
export const LCM_CURRENT_PRERELEASE_TAG = "v7.7.3-lcm.1"
export const LCM_PRERELEASE_CHANGELOG = [
  {
    "title": "Continuation-focused summaries",
    "body": "Summaries now preserve the user intent, permissions, decisions, exact working details, verified progress, and unresolved work. Long sessions keep the information needed to continue implementation instead of spending their summary budget on a narrative recap."
  },
  {
    "title": "Copy-ready recovery citations",
    "body": "lcm_grep supplies UTF-8-safe copy-ready citation ranges to the isolated lcm_query worker, avoiding manual byte-offset arithmetic. Semantic recovery normalizes complete structural scopes and preserves verified events through bounded shard repair while keeping remaining coverage and ordering uncertainty visible."
  },
  {
    "title": "Complete-request hard fitting and stable frontier order",
    "body": "Hard maintenance measures the complete rendered provider request and keeps reducing eligible context while progress is possible. Irreducible requests still fail closed. Frontier publication order uses insertion order rather than timestamps or random identifiers, keeping incremental summary construction deterministic."
  },
  {
    "title": "Native Kilo v7.7.3 integration",
    "body": "LCM actively reuses native transient-part classification, typed tool-validation errors, board configuration, recall ranking and indexes, and the Models compaction selector. Native goals, wakeups, permissions, provider calls, and cancellation stay upstream-owned; locked recovery workers cannot use those tools as an evidence side channel."
  },
  {
    "title": "Settings and raw chats remain compatible",
    "body": "The public soft threshold remains 60%, with the existing 15% clamped recent tail and default recovery budgets. The derived cache remains schema v14/tree v11; Kilo SQLite remains the raw conversation source, and this update introduces no raw-chat migration or forced cache-version rebuild."
  }
]
...
export function validatePrereleaseChangelog(input) {
  ...
  if (
    ...
    !/^v\d+\.\d+\.\d+-lcm\.\d+$/.test(previousTag)
  )
    throw new Error(`Previous prerelease tag ${previousTag} is not a valid LCM release tag`)
...
export function validatePrereleaseNotesBody(body, input = {}) {
  ...
  for (const marker of [`Kilo v${base}`, "lcm_query", "60%", "schema v14/tree v11", previousTag].filter(Boolean))
...
export function renderPrereleaseNotes(input) {
  ...
  const changeHeading = previousTag
    ...
    : `## What LCM adds to upstream Kilo v${base}`
  const body = [
    `This prerelease updates Conversation Memory on Kilo v${base}. It keeps long chats useful by summarizing older, already-used context into a searchable tree while preserving recent work exactly.`,
    "",
    ...
    "",
    "The latest focused Qwen3.8-alias 100k diagnostic recovered an omitted venue reference using one child query and one grep, with an independently verified 512-byte citation; the retained legacy observation was incorrect. All-in logical input was 1,342,560 versus 1,639,590, but LCM used more fresh input and output, so this is not a universal cost-saving claim. A 32k stress retry succeeded, while 13 of 18 summary attempts hit output limits. Semantic ordering and generality to natural work remain limitations. No new full benchmark was run for this port, and parameter comparisons do not justify changing defaults.",
    "",
    "Focused port checks cover native transient-part filtering, typed and message-only tool errors, hidden-worker permissions and ordinary tool continuation, recall compatibility, VS Code surfaces, and generated API contracts. The publication workflow gates release creation on canonical package typechecks, contract stability, all CLI/VSIX builds, and extracted Linux x64 packaged smoke. The benchmark findings above are retained diagnostic evidence, not a claim of reliable complete semantic recovery or optimal parameters.",
    "",
...
export async function verifyGeneratedDocs() {
  const file = "packages/kilo-docs/pages/code-with-ai/platforms/cli-reference.md"
  const { stdout: original } = await execFile("git", ["show", `HEAD:${file}`], { maxBuffer: 8 * 1024 * 1024 })
  const actual = await readFile(file, "utf8")
  validateGeneratedDocs(original, actual)
  await writeFile(file, original)
  console.log("Generated contracts checked; retained the two upstream cloud documentation notes")
}

export function validateGeneratedDocs(original, actual) {
  const expected = original
    .replace("Provide exactly one of `--prompt` or `--prompt-stdin`. For example, read a multiline prompt from a file with `kilo cloud start --repo owner/repo --prompt-stdin < prompt.txt`.\n\n", "")
    .replace("Provide exactly one of `--prompt` or `--prompt-stdin`, as with `kilo cloud start`.\n\n", "")
  if (actual !== expected && actual !== original) throw new Error("Unexpected generated CLI documentation drift")
}
...
export async function runCommand(options, input = {}) {
  ...
  if (options.command === "verify-generated-docs") return verifyGeneratedDocs()
