...
export const LCM_PREVIOUS_PRERELEASE_TAG = "v7.5.15-lcm.1"
export const LCM_CURRENT_PRERELEASE_TAG = "v7.5.15-lcm.2"
export const LCM_PRERELEASE_CHANGELOG = [
  {
    title: "Branch-aware summary recovery",
    body: "Summary-scoped recovery now preserves the selected branch overview and its immediate ordered children before overlapping lexical descendants. Bounded summary labels expose the tree level, child count, and source span, so a locked lcm_query worker can descend into a relevant branch without treating a summary omission as proof that detail is absent.",
  },
  {
    title: "Root promotion cannot starve",
    body: "At a pressure-triggered soft checkpoint, an overdue summary-root promotion now takes its fair one-call quantum before another raw leaf is generated. Continuous eligible raw arrivals can no longer indefinitely postpone construction of a useful multi-level tree, without adding paid work below the existing context-pressure threshold.",
  },
  {
    title: "Complete structural navigation survives display limits",
    body: "Structural pairing now uses the complete consumed-boundary index before independently limiting displayed anchors, and nested units are ordered by their openings. Later complete units therefore remain available to recovery even when a compact navigation display reaches its cap; paired opening and closing bounds stay chronologically coherent.",
  },
  {
    title: "Recovery distinguishes units from event order",
    body: "Private recovery now keeps requested event rank and direction separate from ordering keys and structural-unit selectors. A complete transport range is no longer mistaken for complete semantic-unit coverage. This avoids suppressing the existing shard, reducer, and audit path when a focused question asks for a first or last event in a named unit.",
  },
  {
    title: "Unchanged user settings and raw chats",
    body: "The public soft threshold remains 60%, and recovery budgets, isolation, answer/citation bounds, and ordinary upstream tools are unchanged. The derived cache remains schema v14/tree v11: existing Kilo SQLite chats remain the raw source of truth and this update needs no raw-conversation migration.",
  },
]
...
export function validatePrereleaseNotesBody(body, input = {}) {
  ...
  for (const marker of ["Kilo v7.5.15", "lcm_query", "60%", "schema v14/tree v11", "v7.5.15-lcm.1"])
...
export function renderPrereleaseNotes(input) {
  ...
  const body = [
    ...
    "",
    "Focused qwen3.8 long-session research remains mixed. A post-fix 175k diagnostic returned all four requested values and scored 0.75, up from 0.25 in its earlier same-case trace, but all six isolated answers remained partial or withheld and the parent guessed one value. The maintained contexts also differed, and the question took about 39 minutes. This does not establish reliable complete recovery, a deterministic quality gain, or optimal settings. No new full benchmark was run; 60% remains a research starting point, and subsequent parameter comparisons must inspect actual projected context, answer support, and complete parent/child token usage.",
    "",
    "The product head is `8172e9be16ddbcbfebdca32b17eef013ad6a61be`. Focused checks covered branch-aware tree recovery, root-promotion scheduling, structural navigation, recovery scope contracts, isolation and token accounting, maintenance policy, final prompt ownership, documentation, and public contracts. The publication workflow additionally gates release creation on canonical affected-package typechecks, contract stability, CLI/VSIX builds, and packaged Linux x64 Conversation Memory smoke. The research trace above is a limitation report, not evidence that a benchmark or parameter search is complete.",
    "",
