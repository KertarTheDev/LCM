...
export const LCM_PREVIOUS_PRERELEASE_TAG = "v7.4.23-lcm.12"
export const LCM_CURRENT_PRERELEASE_TAG = "v7.4.23-lcm.13"
export const LCM_PRERELEASE_CHANGELOG = [
  {
    title: "Complete bounded semantic evidence",
    body: "An explicitly bounded `lcm_expand_query` can use up to 50% of usable model input, capped at 64k tokens, while unscoped exploration keeps the smaller 20%/16k limit. Deterministic water-filling redistributes allocation left by short records, so a long range is not clipped merely because another selected record was short. This lets document-, episode-, and section-level questions preserve their full exact scope without turning broad recovery into an unbounded request.",
  },
  {
    title: "Honest answers and bounded transient recovery",
    body: "A generated `coverage: none` response with a blank answer is no longer accepted as successful synthesis. A transient semantic provider failure now directs one identical exact retry before bounded grep/read verification; other fallback causes continue to recommend a genuinely narrower query.",
  },
  {
    title: "Clearer first/last event reasoning",
    body: "Semantic synthesis now builds an ordered event ledger for boundary-sensitive questions and distinguishes an actual new event from a later mention, rejected attempt, or reuse of an ongoing effect. This improves general timeline, audit, and reference-data recovery without changing the five-tool contract.",
  },
]
...
export function validatePrereleaseNotesBody(body, input = {}) {
  ...
  for (const marker of ["lcm_expand_query", "50%", "64k", "coverage: none", "ordered event ledger"])
...
export function renderPrereleaseNotes(input) {
  ...
  const body = [
    ...
    "The published `.12` qwen3.8 175k LCM-only baseline scored `0`, `0.25`, and `0`. The exact-scope candidate canary improved the cross-episode list to `0.5` with full non-truncated semantic inputs, but the final three-question repeat produced one malformed-tool task failure and two zero scores while reporting zero product findings, healthy LCM state, no legacy summaries, and no context integrity loss. Retained traces attribute the remaining misses to provider/model tool-call variance, repeated manual search after decisive evidence, and a benchmark cast-definition ambiguity; this release does not claim a deterministic score gain.",
    "",
    "Focused and integration LCM verification passed for product commit `e63a527f31f6f819628f2a733fb7e565a9e1c266`. A local clean exact-SHA release typecheck stopped safely under the VPS resource guard and was not resumed; this release workflow must complete the canonical package typechecks, clean second contract generation, all 12 CLI and eight VSIX builds, and packaged Linux x64 Conversation Memory smoke before publication.",
    "",
    "Upgrade note: these recovery-interface changes do not alter raw conversation storage or require a sidecar schema migration. Existing Kilo chats remain in SQLite and derived LCM state remains rebuildable.",
    "",
    `Tag: \`${tag}\``,
