...
const onboardingLocales = {
  en: {
    ...
    setup:
      "Connect your usual provider and select a model first. Conversation Memory is on by default. In the extension, open Settings → Experimental to confirm it, then Settings → Context for the 60% soft threshold. Custom models need positive context and output token limits; an input limit is optional.",
    ollama:
    ...
    tips: "Summary work uses model calls, so it can add a little latency and provider cost. Start with 60%; lower it for earlier maintenance or raise it for fewer calls. LCM memory belongs to the current chat. Context export can contain sensitive prompts and tool output, so treat it like the conversation itself.",
    trouble:
  ...
  ar: {
    ...
    tips: "يستخدم التلخيص استدعاءات للنموذج، لذلك قد يضيف قليلاً من التأخير والتكلفة. ابدأ بـ60%؛ اخفضها للصيانة المبكرة أو ارفعها لتقليل الاستدعاءات. ذاكرة LCM تخص المحادثة الحالية. قد يحتوي تصدير السياق على مطالبات ومخرجات أدوات حساسة، فتعامل معه مثل المحادثة.",
    trouble:
  ...
  bn: {
    ...
    setup:
      "প্রথমে আপনার নিয়মিত provider যুক্ত করে একটি model বেছে নিন। Conversation Memory ডিফল্টভাবেই চালু থাকে। extension-এ Settings → Experimental থেকে সেটি যাচাই করুন, তারপর Settings → Context-এ 60% শুরুর সীমা দেখুন। custom model-এর context ও output token limit অবশ্যই ধনাত্মক হতে হবে; input limit দেওয়া ঐচ্ছিক।",
    ollama:
    ...
    tips: "Summary তৈরিতে model call লাগে, তাই সামান্য সময় ও provider cost বাড়তে পারে। 60% দিয়ে শুরু করুন; আগে maintenance চাইলে কমান, কম call চাইলে বাড়ান। LCM memory শুধু বর্তমান chat-এর। Context export-এ sensitive prompt বা tool output থাকতে পারে, তাই chat-এর মতোই নিরাপদ রাখুন।",
    trouble:
  ...
  br: {
    ...
    setup:
      "Conecte o provedor que você já usa e escolha um modelo. Conversation Memory vem ativada por padrão. Na extensão, confirme em Settings → Experimental e abra Settings → Context para ajustar o ponto de início, que começa em 60%. Modelos personalizados precisam de limites positivos de contexto e de saída; o limite de entrada é opcional.",
    ollama:
    ...
    tips: "Os resumos usam chamadas de modelo, então podem acrescentar um pouco de tempo e custo. Comece com 60%; diminua para manutenção mais cedo ou aumente para menos chamadas. A memória do LCM pertence à conversa atual. A exportação de contexto pode conter prompts e saídas sensíveis.",
    trouble:
  ...
  bs: {
    ...
    tips: "Sažeci koriste pozive modelu pa mogu dodati malo vremena i troška. Počnite sa 60%; smanjite za ranije održavanje ili povećajte za manje poziva. LCM memorija pripada trenutnom razgovoru. Izvoz konteksta može sadržati osjetljive upite i izlaz alata.",
    trouble:
  ...
  da: {
    ...
    tips: "Resuméarbejde bruger modelkald og kan derfor give lidt ekstra ventetid og pris. Start med 60%; sænk for tidligere vedligeholdelse eller hæv for færre kald. LCM-hukommelse hører til den aktuelle chat. Konteksteksport kan indeholde følsomme prompts og værktøjsoutput.",
    trouble:
  ...
  de: {
    ...
    tips: "Zusammenfassungen verwenden Modellaufrufe und können etwas Zeit und Kosten hinzufügen. Beginne mit 60%; senke den Wert für frühere Wartung oder erhöhe ihn für weniger Aufrufe. LCM-Speicher gehört zum aktuellen Chat. Ein Kontextexport kann vertrauliche Prompts und Werkzeugausgaben enthalten.",
    trouble:
  ...
  es: {
    ...
    tips: "Los resúmenes usan llamadas al modelo, así que pueden añadir algo de tiempo y coste. Empieza con 60%; bájalo para mantenimiento más temprano o súbelo para menos llamadas. La memoria LCM pertenece al chat actual. La exportación de contexto puede contener prompts y salidas sensibles.",
    trouble:
  ...
  fr: {
    ...
    tips: "Les résumés font des appels au modèle et peuvent donc ajouter un peu de délai et de coût. Commencez à 60 % ; baissez pour intervenir plus tôt ou augmentez pour moins d'appels. La mémoire LCM appartient à la discussion actuelle. Un export de contexte peut contenir des prompts et sorties sensibles.",
    trouble:
  ...
  gr: {
    ...
    tips: "Οι περιλήψεις χρησιμοποιούν κλήσεις προς το μοντέλο, οπότε μπορεί να προσθέσουν λίγο χρόνο και κόστος. Ξεκινήστε με 60%· μειώστε το για νωρίτερη επεξεργασία ή αυξήστε το για λιγότερες κλήσεις. Η μνήμη LCM ανήκει στην τρέχουσα συνομιλία. Η εξαγωγή περιβάλλοντος μπορεί να περιέχει ευαίσθητες προτροπές και αποτελέσματα εργαλείων.",
    trouble:
  ...
  it: {
    ...
    setup:
      "Collega il provider che usi di solito e scegli un modello. Conversation Memory è attiva per impostazione predefinita. Controlla in Settings → Experimental, poi apri Settings → Context; la soglia iniziale è impostata al 60%. I modelli personalizzati richiedono limiti positivi per contesto e output; il limite di input è facoltativo.",
    ollama:
    ...
    tips: "I riepiloghi usano chiamate al modello, quindi possono aggiungere un po' di tempo e costo. Parti dal 60%; abbassa per intervenire prima o alza per meno chiamate. La memoria LCM appartiene alla chat corrente. L'export del contesto può contenere prompt e output sensibili.",
    trouble:
  ...
  ja: {
    ...
    tips: "要約にはモデル呼び出しを使うため、少し時間と料金が増えることがあります。まずは 60% のまま使い、早めに整理したい場合は下げ、呼び出し回数を減らしたい場合は上げます。LCM の記憶は現在のチャット専用です。コンテキストの書き出しには、機密性の高いプロンプトやツール出力が含まれる場合があります。",
    trouble:
  ...
  ko: {
    ...
    tips: "요약에는 모델 호출이 사용되어 약간의 지연과 제공자 비용이 더 생길 수 있습니다. 60%로 시작하고 더 일찍 정리하려면 낮추고 호출을 줄이려면 높이세요. LCM 메모리는 현재 채팅에만 속합니다. 컨텍스트 내보내기에는 민감한 프롬프트와 도구 출력이 들어갈 수 있습니다.",
    trouble:
  ...
  no: {
    ...
    tips: "Sammendrag bruker modellkall og kan gi litt ekstra tid og kostnad. Start med 60%; senk for tidligere vedlikehold eller hev for færre kall. LCM-minnet tilhører gjeldende chat. Konteksteksport kan inneholde følsomme ledetekster og verktøyresultater.",
    trouble:
  ...
  pl: {
    ...
    setup:
      "Najpierw podłącz używanego na co dzień dostawcę i wybierz model. Conversation Memory jest domyślnie włączona. Sprawdź to w Settings → Experimental, a następnie otwórz Settings → Context; próg początkowy wynosi 60%. Modele niestandardowe wymagają dodatnich limitów kontekstu i wyjścia, natomiast limit wejścia jest opcjonalny.",
    ollama:
    ...
    tips: "Podsumowania używają wywołań modelu, więc mogą dodać trochę czasu i kosztu. Zacznij od 60%; obniż dla wcześniejszej pracy lub podnieś dla mniejszej liczby wywołań. Pamięć LCM należy do bieżącej rozmowy. Eksport kontekstu może zawierać poufne prompty i wyniki narzędzi.",
    trouble:
  ...
  th: {
    ...
    tips: "การสรุปต้องเรียกใช้โมเดล จึงอาจเพิ่มเวลาและค่าใช้จ่ายเล็กน้อย เริ่มที่ 60% ลดค่าหากต้องการให้จัดการเร็วขึ้น หรือเพิ่มค่าเพื่อลดจำนวนครั้งที่เรียกโมเดล หน่วยความจำ LCM เป็นของแชตปัจจุบัน การส่งออกบริบทอาจมีพรอมต์และผลลัพธ์จากเครื่องมือที่เป็นข้อมูลสำคัญ",
    trouble:
  ...
  tr: {
    ...
    setup:
      "Önce her zamanki sağlayıcınızı bağlayın ve bir model seçin. Conversation Memory varsayılan olarak açıktır. Bunu Settings → Experimental bölümünde kontrol edin, ardından Settings → Context bölümünü açın; başlangıç eşiği %60'tır. Özel modellerde bağlam ve çıktı sınırları pozitif olmalıdır; giriş sınırı isteğe bağlıdır.",
    ollama:
    ...
    tips: "Özetleme model çağrıları kullanır; biraz gecikme ve maliyet ekleyebilir. %60 ile başlayın; daha erken bakım için azaltın, daha az çağrı için artırın. LCM belleği mevcut sohbete aittir. Context export hassas prompt ve araç çıktıları içerebilir.",
    trouble:
  ...
  vi: {
    ...
    tips: "Việc tóm tắt cần gọi mô hình nên có thể tốn thêm một chút thời gian và chi phí. Hãy bắt đầu với 60%; giảm xuống để xử lý sớm hơn hoặc tăng lên để giảm số lần gọi. Bộ nhớ LCM chỉ thuộc cuộc trò chuyện hiện tại. Bản xuất ngữ cảnh có thể chứa câu lệnh và kết quả công cụ nhạy cảm.",
    trouble:
  ...
  zh: {
    ...
    setup:
      "先连接你常用的服务商并选择模型。Conversation Memory 默认开启，可在 Settings → Experimental 中确认。然后打开 Settings → Context；默认从 60% 开始处理。自定义模型的上下文上限和输出令牌上限必须为正数，输入上限可以不填。",
    ollama:
    ...
    tips: "生成摘要需要调用模型，因此可能增加少量延迟和费用。建议先用 60%；想更早处理可调低，想减少调用可调高。LCM 记忆仅属于当前聊天。上下文导出可能包含敏感的提示词和工具输出，请像保护聊天内容一样妥善保管。",
    trouble:
  ...
  zht: {
    ...
    setup:
      "先連接常用的服務供應商並選擇模型。Conversation Memory 預設開啟，可在 Settings → Experimental 中確認。接著開啟 Settings → Context；預設會從 60% 開始處理。自訂模型的內容上限與輸出權杖上限必須是正數，輸入上限可不填。",
    ollama:
    ...
    tips: "產生摘要需要呼叫模型，因此可能增加少量等待和費用。建議先用 60%；想提早處理就調低，想減少呼叫就調高。LCM 記憶只屬於目前聊天。內容匯出可能含有敏感的提示詞與工具輸出，請像保護聊天內容一樣妥善保管。",
    trouble:
...
export const LCM_PREVIOUS_PRERELEASE_TAG = null
export const LCM_CURRENT_PRERELEASE_TAG = "v7.5.15-lcm.1"
export const LCM_PRERELEASE_CHANGELOG = [
  {
    title: "Direct Kilo v7.5.15 port",
    body: "Conversation Memory is rebuilt directly on the exact upstream Kilo v7.5.15 tag. The port preserves upstream shared-agent boards, improved single-turn subagent pruning, continued subagent execution after permission denial, and corrected provider request headers. LCM still owns the missing summary tree, active-context projection, and isolated current-session recovery path; boards remain coordination rather than raw-memory recovery.",
  },
  {
    title: "Configurable maintenance at a 60% starting point",
    body: "The public `conversation_memory.soft_threshold_percent` default remains 60%, as in the preceding v7.5.9 prerelease. The exact recent tail, successful-consumption protection, hard-overflow convergence, and fail-closed behavior remain intact. Advanced recovery budgets are configurable independently of shared boards. The 60% value is a research starting point, not a proven optimum; this upstream port does not claim completed parameter optimization.",
  },
  {
    title: "Bounded isolated detail recovery",
    body: "The ordinary agent sees only `lcm_query`. A locked hidden child answers the exact focused question using prior-turn memory and returns only a bounded answer with optional host-verified citations. Prerequisite lookups and independent questions can use the configured budget even after another question was fully answered. The surrounding task cannot override the focused assignment, and a preceding bounded result is optional context rather than a mandatory narrowing rule. Both hidden phases remain excluded from board tools and notifications, and from explicitly enabled end-of-turn pruning.",
  },
  {
    title: "Memory budgets no longer stop ordinary agent work",
    body: "Exhausting recovery queries no longer removes ordinary tools, forces an answer-only response, or terminates the parent turn. The agent can continue inspecting files, implementing changes, and testing with already recovered requirements. Duplicate questions and configured resource ceilings remain enforced without exposing private evidence. Recovery budgets are configurable; their current defaults are not claimed to be optimal.",
  },
  {
    title: "Disposable derived-state rebuild",
    body: "The derived cache remains schema v14/tree v11. Older incompatible sidecars rebuild from Kilo SQLite, the sole raw transcript source of truth; compatible current sidecars do not need a schema change just for this upstream port. Rebuilding derived state does not migrate, rewrite, or delete chats.",
  },
]
...
export function validatePrereleaseChangelog(input) {
  ...
  if (
    ...
    (!/^v\d+\.\d+\.\d+-lcm\.\d+$/.test(previousTag) || !previousTag.startsWith(`v${base}-lcm.`))
  )
...
export function validatePrereleaseNotesBody(body, input = {}) {
  ...
  const base = input.base ?? "7.5.15"
  const heading = previousTag
    ? `## What changed since ${previousTag}`
    : `## What LCM adds to upstream Kilo v${base}`
  if (!body.includes(heading)) throw new Error(`Prerelease notes must identify the concrete delta in ${heading}`)
  for (const marker of ["Kilo v7.5.15", "lcm_query", "60%", "schema v14/tree v11"])
    if (!body.includes(marker)) throw new Error(`Prerelease notes are missing concrete LCM delta marker: ${marker}`)
  if (!body.includes("## Evidence and limitations"))
    throw new Error("Prerelease notes must report evidence and limitations without requiring a benchmark-specific size")
  return body
}
...
export function renderPrereleaseNotes(input) {
  ...
  const changeHeading = previousTag
    ? `## What changed since ${previousTag}`
    : `## What LCM adds to upstream Kilo v${base}`
  const body = [
    ...
    "",
    "The retained qwen3.8 175k research traces remain mixed. One later source-attested sample scored 1.0 on the four requested values, but its explanation attributed one event to the wrong recipient and both recovery children reported partial coverage. Earlier comparable canaries scored 0.5 and 0.75. These results do not establish reliable complete recovery or a deterministic score gain. No new full benchmark was run during this upstream port; 60% remains a research starting point, and subsequent parameter comparisons must inspect actual projected context, answer support, and complete parent/child token usage.",
    "",
    "The product head is `88c4db6b45d649e52533b58b2c948541cfdd1669`, with purposeful recovery corrections above the direct upstream v7.5.15 port. Local focused checks covered cross-domain prerequisite and independent-query admission, ordinary tool execution after recovery exhaustion, board isolation, both hidden-worker pruning paths, processor overflow ownership, configuration, exports, generated contracts, and adapted VS Code behavior and lint. Two SDK/OpenAPI generations produced identical hashes, and committed upstream annotations passed. The publication workflow additionally gates release creation on canonical affected-package typechecks, contract stability, CLI/VSIX builds, and packaged Linux x64 Conversation Memory smoke. The historical model traces above predate these latest corrections; they are not evidence of a new score gain.",
    "",
    "Upgrade note: existing Kilo chats remain in SQLite. Pre-v7 LCM derived caches are disposable and rebuild automatically as schema v14/tree v11; no raw conversation migration is performed.",
    "",
    `Tag: \`${tag}\``,
  ...
  return validatePrereleaseNotesBody(body, { base, previousTag })
}
