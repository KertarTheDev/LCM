...
export const LCM_CURRENT_PRERELEASE_TAG = "v7.5.9-lcm.1"
export const LCM_PRERELEASE_CHANGELOG = [
  {
    title: "Direct Kilo v7.5.9 port",
    body: "Conversation Memory is rebuilt directly on the exact upstream Kilo v7.5.9 tag. It reuses Kilo's current compaction-model selection, incomplete-response retry and output caps, per-user editor context, and newer drain/cancel/queue lifecycle instead of carrying parallel host behavior. LCM still owns only the missing summary tree, active-context projection, and current-session recovery path.",
  },
  {
    title: "Less premature maintenance by default",
    body: "The public `conversation_memory.soft_threshold_percent` default moves from 40% to 60%. This leaves more already-useful provider context in place before soft summary work starts, reducing extra model calls and cache churn in ordinary long sessions while preserving the same exact recent tail, hard-overflow convergence, and fail-closed safety. The value remains configurable, and 60% is the starting point for continued parameter research rather than a claim that optimization is finished.",
  },
  {
    title: "Bounded isolated detail recovery",
    body: "The ordinary agent sees only `lcm_query` and asks one focused question. A locked hidden child receives the exact prior-turn memory plus the five recovery primitives, does the heavy exact or semantic retrieval, and returns only a bounded answer with optional host-verified citations. This preserves raw detail without injecting full recovered transcripts or model-selectable session handles into the main context.",
  },
  {
    title: "Disposable derived-state rebuild",
    body: "Existing pre-v7 Conversation Memory sidecars are intentionally rebuilt as schema v14/tree v11 so no stale or length-truncated summary is reused across the upstream upgrade. Kilo SQLite remains the sole raw transcript source of truth; the rebuild does not migrate, rewrite, or delete chats.",
  },
]
...
export function validatePrereleaseNotesBody(body, input = {}) {
  ...
  for (const marker of ["Kilo v7.5.9", "lcm_query", "60%", "schema v14/tree v11"])
...
export function renderPrereleaseNotes(input) {
  ...
  const body = [
    ...
    "The published v7.4.23 `.12` qwen3.8 175k LCM-only baseline scored `0`, `0.25`, and `0`. The `.13` exact-scope canary improved one cross-episode list to `0.5`, but its final three-question repeat still had one malformed-tool task failure and two zero scores while reporting zero product findings, healthy LCM state, no legacy summaries, and no context-integrity loss. Those traces support the model-neutral recovery contracts carried here, but they do not establish a deterministic score gain. No new full benchmark was run during this upstream port; 60% is a cost-informed starting point for the resumed research.",
    "",
    "Focused and integration LCM verification passed for product commit `5e7df6a67c55e5c43c2bbbb488df9c7eea2e7b0d`, including 357 OpenCode assertions, 144 VS Code assertions, isolated queue/remote/processor suites, and the focused recovery/type-boundary regression suites, plus generated-contract stability, upstream-compatibility coverage, and the long-context gate. A local clean exact-SHA OpenCode typecheck emitted no diagnostics before the VPS resource guard stopped it and, by policy, was not resumed. Canonical attempts then exposed and drove fixes for seven cross-file inference/discriminant errors, state leakage from batching independent test files in one process, and a stale packaged-smoke expectation that contradicted the isolated `lcm_query` contract. This release workflow must complete the corrected canonical package typechecks, clean second contract generation, all 12 CLI and eight VSIX builds, and packaged Linux x64 Conversation Memory smoke before publication.",
    "",
