...
The fork job verifies exact product ancestry and a six-file release-only overlay, then runs only the prompt/queue/CLI,
route/contract, long-context fixture, TUI, VS Code, compiler, and generated-drift gates affected by the current upstream
port. Existing upstream and unchanged Conversation Memory suites are trusted. It builds 12 CLI archives and eight VSIX
files, runs smoke against the extracted Linux VSIX, and only then creates a draft release targeting the exact workflow
SHA.
