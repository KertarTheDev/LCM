...
This document maps the current release-sync LCM branch against upstream release `v7.4.15` (`13e425966261e159e915903df5a9e54792a0c657`). It describes the ownership boundaries after the LCM subsystem was replayed onto seams audited on the target release.
...
- `map.ts`: `llm_map`, `agentic_map`, created/resumed start snapshots, observational map status/cancel, immutable JSONL input snapshots, runtime-owned map identity/progress, restart/owning-session recovery, leases, typed failure classification, retry, and output artifacts.
- `agentic-map-runner.ts`: production Kilo instance/runtime bridging, DB-proven map-child registration, restricted child permissions, normal prompt/provider execution, and trusted `StructuredOutput` finalization.
- `agentic-map-runner.ts`: reconstructable full Kilo child-session execution, trusted prompt/finalizer boundary, tool permissions, abortable child-slot ownership, and structured-output adaptation.
- `runtime-provider.ts`, `provider-protocol.ts`, `provider-capacity.ts`, `provider-overhead.ts`, `model-limits.ts`, `preflight-errors.ts`, `render-prep.ts`, `token-budget.ts`, `summary.ts`: typed map execution planning, provider transform/protocol validation, fair local endpoint admission for both map kinds, provider-transform overhead reserve sizing, model-limit fallback/recovery windows, preflight safe-action classification, render manifests, deterministic token counting, and summary quality/fallback behavior.
...
- `packages/opencode/src/session/prompt.ts`: treats product V1 sessions as construction-time LCM-owned, loads the full durable stream and runtime-proven conversation capability, rejects non-runnable lifecycle states before provider work, delegates preflight/provider-safe assembly with that same capability class, gates LCM tools by capability, finalizes provider request snapshots, and never selects legacy compacted history as a recovery mode.
...
- `packages/kilo-vscode/webview-ui/src/components/chat/TaskHeader.tsx`, `TaskUsage.tsx`, and `lcm-status.ts`: active-session LCM event/status hydration plus monotonic hard/raw/backlog pressure inside the collapsible upstream token-usage details below the timeline.
...
- `packages/kilo-vscode/webview-ui/src/components/chat/TaskTimeline.tsx` and `lcm-timeline.ts`: timestamp-merge provider-backed LCM maintenance, retrieval, exploration, and map token activity into the normal Kilo timeline with upstream bar geometry while excluding non-provider status bookkeeping.
