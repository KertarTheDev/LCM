...
Completed tool parts may carry truncation sidecar metadata from the runtime tool wrapper. Source sync validates the metadata version, path under the runtime `tool-output` directory, byte count, and SHA-256 as recovery/provenance evidence, but it does not silently replace the persisted result with current sidecar bytes. The canonical immutable LCM source is the final persisted MessageV2 ToolPart after upstream SWE pruning. Full path-backed reads require an explicit provenance-checked admission path. If sidecar validation fails or the sidecar is missing, the durable visible output/error remains valid canonical source and no untrusted filesystem bytes are substituted.
...
- The first-release checkpoint pass is awaited with foreground provider/DB priority; only deferred retry work runs in the background.
...
Soft maintenance never summarizes the current user turn being answered. It may summarize older consumed rows from the same multi-step assistant turn once a later resolved provider request proves those rows were model-visible and they fall outside the runtime-owned fresh-tail budget.
...
Provider request snapshots also record the ordered context items represented by each source-backed render unit. Snapshot context-item IDs are immutable historical evidence even when active context is rewritten. The snapshot header and ordered item rows are inserted atomically, including the valid zero-item case. Finalizing a snapshot as `resolved` atomically terminalizes it and inserts first-consumption evidence keyed by included durable raw message rows; a consumption-write failure leaves the snapshot in flight for retry. Canceled, expired, or failed requests do not create consumption evidence. This evidence is used only for future soft-backlog eligibility and is not a provider wire log.
...
`writeContextSnapshot(...)` records active context state with a `lcm-context-restore-manifest-v2` restore manifest and provider-safe metrics. `rebuildActiveContext(...)` first tries durable snapshots and then rebuilds from durable source, summary lineage, archive pointers, and standalone large-file markers. Rebuild activates only summary roots that have not themselves been replaced by a newer condensation, recursively treats their descendant message rows as covered, ignores finalized metadata-only messages with no source parts, and fails closed unless every stored summary belongs to the conversation, has direct same-conversation provenance, is reachable from an active root, is acyclic, and stays within the bounded lineage depth.

Snapshot restore is all-or-nothing per snapshot. The manifest's non-cue durable keys must be an exact unique-set match for the current durable projection, so an older manifest cannot omit later source or retain a superseded summary. Corrupt, stale, or incompatible snapshots are skipped. Active context is derived state and can be rebuilt when immutable source and summary records are complete.

Threshold-to-assembly cache reuse requires the exact render-preparation identity and threshold-decision hash for the same conversation. Before token-cache, threshold-snapshot, or provider-snapshot writes, the serialized DB operation revalidates lifecycle, boundary, capability, lineage/scope, derived strategy, active-context rows, reconstructed raw leaves, large-file marker text, consumption evidence, and the provider-transform overhead reserve that shaped the decision. It also rechecks the threshold source hash immediately before persistence and refreshes request-snapshot protection evidence from the revalidated state. Any source, authority, decision, budget, or cancellation mismatch returns a content-safe stale/canceled error and rolls back all threshold and provider snapshot writes.
...
The exporter reconstructs active-context Markdown from durable context rows or context snapshots. The initial dialog uses the newest snapshot before the first summary, or current context when no such summary exists. For each durable summary replacement, it emits a separate LCM prompt file for the summary request and starts the next ordered dialog from the first replacement snapshot at or after that summary when available. Dialog files include rendered raw messages, summaries, large-file markers, retrieval cues, and terminal tool input/output/error text that is stored in LCM but normally hidden from the chat UI.
...
When capability-scoped LCM tools are available, main-turn system context starts with one short LCM policy covering cross-tool trust and exact-detail recovery. The complete upstream Kilo sequence remains intact and contiguous after it: environment blocks, upstream project-memory context, project/configured instructions, then the available-skill catalogue. A structured-output directive, when required, remains the final conditional system-context block. Project-memory text remains external project knowledge and is budgeted as model-visible context; it is not adopted as immutable LCM lineage source. Per-tool workflow, parameters, authorization, polling, and recovery guidance live only in the provider-native tool descriptions and schemas; the system policy does not enumerate tools or duplicate those descriptions.
...
If the provider rejects the final transformed payload for context length after preflight approved the request, an LCM-active session performs one recovery preflight and one physical retry with a stricter effective provider input reserve. Failed assistant placeholders from that transient provider overflow are not synced into LCM source, and transient provider overflow events are suppressed so clients see either the recovered response or the final content-safe LCM error.
