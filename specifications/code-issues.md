...
GitHub workflow run `30072363434` published `v7.4.15-lcm.4` as release `359118859` from exact prerelease head `15ebfcc659e81cb973c625227e00049ba3b4b5c1`. Independent verification proved the tag/SHA binding and all 20 required nonempty assets. Packaged Linux x64 DB/continuation smoke passed and uploaded evidence artifact `8588692692`, including first-turn `lcm_grep` registration for a passive old-session scope and durable 3/3 then 4/4 Core/LCM coverage. After replacement verification, superseded release `359029133` and tag `v7.4.15-lcm.3`, bound to `6a7a1c38ef787c52075c385493c107707f7e6ac1`, were deleted by exact identity; remote verification confirmed `.3` absent and `.4` still exact with its complete asset profile. This refreshes the v7.4.15 prerelease evidence for the local-provider agentic-map capacity fix, but it is not stable-release approval.
