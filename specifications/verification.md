...
GitHub workflow run `30094567166` published prerelease `v7.4.15-lcm.8` as release `359305370` from exact prerelease head `587eaca0fa8e03704078bfc2f398447cf8b241fb`, based on product commit `45ab20ca0d599e399a57070cdf317bf06a5bad4a`. The workflow overlay guard proved product ancestry and limited the prerelease delta to 29 approved release-only files. Contract drift, map, scheduler, sub-agent-scope, and provider-assembly checks passed before artifact creation. The packaged Linux x64 runtime smoke passed and uploaded evidence artifact `8597268056`; CLI and VSIX workflow artifacts are `8597190999` and `8597313330`. Independent verification proved that the published prerelease tag resolves to the exact workflow commit, its description begins `LCM alpha build based on upstream Kilo Code v7.4.15.`, and all 20 required assets—12 CLI archives and 8 platform VSIX files—are nonempty. After replacement verification, faulty release `359240777` and tag `v7.4.15-lcm.7`, bound to `39ffbfb4edd20f2fd434eaff672bcef8d943d87c`, were deleted by exact release ID, tag, and SHA; GitHub returned `404` for both old identities while the `.8` release and tag remained present. This completed prerelease automation evidence is not installed-editor or stable-release approval.
