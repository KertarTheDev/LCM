...
This document records the verification surface for the LCM implementation on the `kilocode-lcm-v7.4.1` branch. It is descriptive of the current code and package scripts.
...
- `lcm:db:support`: runtime DB support smoke coverage, including owner-lock support-command behavior, guarded owner-lock recovery preview/apply, PGlite gate checks, and content-safe diagnose checks for migration registry, search/index readiness, deferred jobs, large-payload markers, path provenance, map rows, and artifact cleanup queue readability.
- DB worker focused file: `bun run --cwd packages/opencode test test/lcm/db-worker.test.ts` covers owner-lock heartbeat, stale takeover, lock conflict, content-safe owner-lock support recovery helpers, post-startup lock-loss fail-closed behavior, DB queue bounds, queue metrics, queued cancellation, prompt-critical request timeout defaults, active request timeout, and shutdown abort/drain behavior.
...
- `lcm:active-context:test`: exact snapshot restore, recursive source coverage, metadata-only finalized-message exclusion, root chronology, complete summary-graph reachability/provenance/cycle/depth validation, malformed cue lineage, and fail-closed rebuild behavior.
- Passive runtime focused file: `bun run --cwd packages/opencode test test/lcm/passive-runtime.test.ts` covers preflight activation without generating fresh retrieval cues for the activating passive turn, prompt-preflight phase status updates and blocked-result cleanup, canceled hard-limit fail-closed behavior, manual/repair result identity across recovery lifecycle states, hard-limit maintenance progress labels, post-turn soft-maintenance retry policy, one-shot stale-threshold-context rebuild, after-turn raw-counter threshold refresh without durable no-op attempt spam, runtime-restart resume of deferred soft maintenance with protected-current-user payload preservation, bounded shutdown handling for already-running deferred retries, terminal metrics-before-maintenance-event publication, and blocking maintenance status cleanup.
- `lcm:crash-reopen`: representative recovery coverage across summary, hard-limit, and map paths.
- `lcm:scheduler`: foreground/background scheduling fairness, workspace-scoped soft-maintenance caps, provider capacity, local endpoint deferral, pre-aborted lane rejection without idle registry allocation, soft-sweep iteration/elapsed budget helpers, and route-level summary failure cooldown/backoff helpers.
...
- Finalized source sync: `bun run --cwd packages/opencode lcm:finalized-sync` covers source ingestion, current-user raw-context re-pin when an existing sealed source row is missing from active context, recursive prevention of raw-context resurrection for messages covered through active bindle lineage, cooperative cancellation, benign assistant accounting metadata drift, rebuild-safe source-drift repair, fail-closed unsafe source drift recovery actions, and post-turn sync warning/retry behavior, including persisted retry recovery after runtime restart.
...
- Prompt render-boundary smoke: `bun run --cwd packages/opencode lcm:prompt-first-message` covers a new plan-session prompt entering the LCM render-preparation path with Kilo render-only reminders and malformed render-only metadata completing with `LcmMemoryError` before any provider request.
...
- `lcm:provider-assembly`: provider-safe assembly output, atomic request-snapshot header/item insertion, immutable snapshot-item evidence across active-context rewrites, atomic resolved-terminalization/consumption rollback and retry, zero-raw terminalization, cooperative assembly cancellation before request snapshots, and runtime prepared-payload guard behavior.
...
- `lcm:assembly-token-budget`: assembly token budget; exact render-preparation and threshold-decision binding; conversation authority, lifecycle, strategy, context, raw/file source, consumption, and provider-overhead revalidation; request-protection refresh; cooperative user/DB-worker cancellation with transaction rollback; and reserve behavior.
...
- `lcm:retrieval-tools`: `lcm_grep`, `lcm_describe`, `lcm_expand`, `lcm_expand_query`, and `lcm_read` behavior, including canonical retrieval workflow descriptions, exact-detail recovery guidance for compressed summaries, searchable large-file marker handles without full-artifact grep, root-safe `summaryID` large-output recovery through `lcm_expand_query`, stale or out-of-scope optional handle fallback, LCM tool-call echo down-ranking in grep results, malformed cross-lineage summary link filtering, memory-cue cancellation, tagged retrieval answer prompt requests, internal structured expand-query answer envelope parsing, provider-backed expand-query request preparation and model-unavailable safe errors, cited extractive fallback with provider diagnostics when provider synthesis fails after excerpts are found, true no-excerpt `coverage = "none"` behavior, and degraded fallback-summary source preference.
...
- VSCode transcript export focused unit test: `bun --cwd packages/kilo-vscode test tests/unit/export-transcript.test.ts` covers cursor-paged transcript export and readable tool input/output/error preview rendering with large-output sidecar metadata.
- VSCode LCM prewarm tests: `bun run --cwd packages/kilo-vscode lcm:prewarm` covers coalesced advisory prewarm, retry invalidation, bounded retry logging noise, timeout diagnostics that do not block advisory prewarm, prompt/command-send readiness waits, wait cancellation, explicit readiness probe safe-error preservation, malformed safe-error-shaped payload rejection, and rejected capability route errors.
- VSCode provider utility tests: `bun --cwd packages/kilo-vscode test tests/unit/kilo-provider-utils.test.ts` covers SSE-to-webview session status forwarding for retry, busy memory-progress messages, and offline recovery messages.
- VSCode network event tests: `bun --cwd packages/kilo-vscode test tests/unit/connection-utils.test.ts tests/unit/network-handler.test.ts` covers typed `session.network.*` session resolution and network-restore auto-reply cleanup.
- VSCode context UI tests: `bun run --cwd packages/kilo-vscode lcm:context-ui`
- VSCode session-utils focused file: `bun --cwd packages/kilo-vscode test tests/unit/session-utils.test.ts` covers stale LCM metrics rejection by `updatedAt` and maintenance hints using terminal post-backlog fields.

Settings tests must verify that sessionless LCM settings calls do not open PGlite, do not fabricate a session, and do not return `lifecycleState` or `dbStatus`, while session-scoped settings calls report runtime-owned `lifecycleState` and `dbStatus` for the active family. They must cover the writable `strategy` and `storageWarningThresholdBytes` fields and reject internal `freshTailTokens` as an unsupported public field. The internal 20,000-token fresh-tail default remains covered by threshold and soft-backlog suites. Settings tests must also verify that malformed runtime safe-error-shaped responses are not forwarded to the webview, and that session DB diagnosis/recover-lock/rebuild/prompt-export derives its family from trusted session state rather than caller-supplied data directories. `lcm:db:support` owns the content-safe DB smoke/diagnose/recover-lock/rebuild report shape, including owner-lock recovery state, owner-lock/layout, migration registry, search/index, deferred-job, large-payload marker, path-provenance, map-row, and artifact-cleanup health checks. Recover-lock tests must cover dry-run preview defaults, explicit-force requirements for malformed or uncheckable locks, forged body rejection, live-owner refusal, and owner.lock-only quarantine. Rebuild tests must cover dry-run preview defaults, forged body rejection, and healthy-family apply refusal. VSCode tests must keep the Memory settings page free of an LCM enable/disable switch, raw memory export, raw memory browse, and LCM-only delete controls, and must cover request-ID handling so stale Memory settings responses cannot overwrite newer state. Safe-action tests must prove only supported actions become buttons, including contact-support links, read-only DB diagnostics, guided owner-lock recovery preview/apply, guided DB repair preview/apply, and prompt export through runtime-owned routes while arbitrary DB reset, raw inspection, and extension-host DB ownership remain absent from the webview. Memory state tests must keep `Export prompts` visible but disabled with a content-safe status for session-backed non-ready DB states, and enabled only when DB status is ready. Memory state tests must also cover inline maintenance progress details for active/deferred maintenance, waiting-checkpoint status under over-soft raw pressure, and separate raw-lane pressure from soft-backlog, fresh-tail, and unconsumed-row pressure. Context UI tests must verify that `lcm.metrics.updated` events are keyed by current session and all conversation identifiers, and that active-budget pressure comes from current-session LCM metrics rather than provider token fallback.
