...
GitHub workflow run `29984959329` published prerelease `v7.4.15-lcm.1` as release `358493732` from exact prerelease head `90609f2a3fd49564f539ef34d18c86f73c3cc308`. Independent verification proved that the tag resolves to that commit and that all 20 required assets—12 CLI archives and 8 platform VSIX files—are nonempty. The packaged Linux x64 runtime smoke passed DB asset loading plus two-process old-session continuation with first-turn `lcm_grep` registration and durable 3/3 then 4/4 Core/LCM coverage. After replacement verification, superseded release `358208124` and tag `v7.4.13-lcm.3` were deleted by exact release ID, tag, and SHA; remote tag verification confirmed the old tag absent and the replacement tag still bound to the expected commit. Historical `v7.4.13` workflow results are not evidence for this branch. This completed prerelease automation evidence is not stable-release approval.
