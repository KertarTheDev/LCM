...
| `llm_map` | An authorized JSONL run is created and returns a durable `map_...` status. Requests with workers `1`, `2`, and omitted must succeed on a local provider with `effectiveWorkers = 1`; a remote provider requested with two workers must retain two. A busy local endpoint must report live `waiting_capacity`, send no overlapping physical request, consume no retry/active budget, and complete automatically without a status-triggered retry. |
| `agentic_map` | An authorized read-only or write-capable run returns its durable handle before child provider work begins, uses a full Kilo child plus the trusted `StructuredOutput` finalizer after any tool work, keeps tagged item/schema content in the user channel, reports `effectiveWorkers`, and completes schema-valid output. Requests with workers `1`, `2`, and omitted must succeed locally with one effective worker. Foreground, direct-map, and map-child physical calls must remain serialized and alternate when both lanes are queued, without an item-wide reservation. A trivial two-item square map must complete after scheduler shutdown/status resume using persisted parent/agent/directory identity. Provider and logical child-slot waiting must appear as `waitingCapacityItems` without advancing active-work time. Typed child/provider errors must retain their classifications; unclassified internal failures use `recovery_failed`, output-length/ignored warning text is not parsed, retryable invalid responses consume only the durable item retry budget, capacity waiting consumes no retry, and cancellation removes logical-slot/provider waiters. |
