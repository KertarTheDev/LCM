...
GitHub workflow run `30057848352` published prerelease `v7.4.15-lcm.3` as release `359029133` from exact prerelease head `6a7a1c38ef787c52075c385493c107707f7e6ac1`. Independent verification proved that the tag resolves to that commit and that all 20 required assets—12 CLI archives and 8 platform VSIX files—are nonempty. The packaged Linux x64 runtime smoke passed and uploaded evidence artifact `8583605306`, proving DB asset loading plus two-process old-session continuation with first-turn `lcm_grep` registration and durable 3/3 then 4/4 Core/LCM coverage. After replacement verification, superseded release `358693627` and tag `v7.4.15-lcm.2`, bound to `19817f8150c8f78af7ff48d40f6f5e94738d1a06`, were deleted by exact release ID, tag, and SHA; remote verification confirmed the old release and tag absent and `.3` still bound to the expected commit with its complete asset profile. Historical `.2`, `.1`, and `v7.4.13` workflow results are not current-candidate evidence. This completed prerelease automation evidence is not stable-release approval.
