...
- `lcm:agentic-map-runtime`: focused production-service integration through root activation, durable agentic start, restricted map-child creation/family registration, loopback provider `StructuredOutput`, terminal map completion, and output artifact creation.
...
Normalize live-audit results by the nine tool identities below, not by the number of scenarios or mode variants exercised. Report each tool exactly once as `passed`, `failed`, or `not_run`, with literal/regex/case/pagination checks nested under `lcm_grep`. Tool registration or availability without an invocation is `not_run`, not `passed`. A model's prose assessment, hidden reasoning, self-authored pass/fail table, statement that a tool was accepted, or inference that an earlier problem was fixed is qualitative usability evidence only; it is never the release executor's verdict. Treat a small-model report as a hint about discoverability, descriptions, and likely failure paths, then grade the captured tool requests/results, durable rows/artifacts, and provider evidence independently. A terminal `provider_invalid_response` for an agentic item proves output rejection only; it is not evidence by itself that a child failed to launch or that the model was the sole faulty component. An expected authorization or cursor-scope rejection counts as `passed` only when the audit deliberately exercised that negative contract and observed the specified safe error; in particular, reusing an `lcm_grep` cursor with another query must return `lcm_retrieval_cursor_scope_mismatch`. Do not describe omitted `lcm_expand`/`lcm_read` checks or an unexercised `lcm_map_cancel` as successful coverage.

| Tool | Required live proof |
|---|---|
| `lcm_grep` | Root session returns authorized literal/regex matches and stable handles. Literal proof must use the exact-contiguous contract; separated-term behavior must be tested with separate literal calls or explicit regex. Every hit must expose `sourceClass`, part-backed hits must expose `partKind`, and direct LCM tool rows alone set `isLcmToolEcho = true`; assistant prose discussing a prior result remains `assistant_content` with `isLcmToolEcho = false`. Original user/tool/file evidence must rank before newer assistant discussion and direct echoes. Follow `page.nextCursor` while `hasMore` is true before recording an absence. A hit that only repeats the current model/tool discussion is not proof that the older sought source was retrieved. |
...
| `lcm_expand_query` | Root session returns a cited answer or the specified successful no-answer shape; activity records provider usage. An explicit summary query must place covered original source ahead of summary prose and recover exact detail on empty-provider fallback. A broad one-term/assistant-discussion match plus empty, malformed, declined, or citation-invalid provider output must return `answer = ""`, `noAnswerReason = "insufficient_relevance"`, and the separate `providerFailureReason`, not a mixed extractive answer. Supported local Ollama/OpenAI/Azure routes must send the fixed trusted output schema. |
...
| `llm_map` | An authorized JSONL start returns `runDisposition`, a durable `map_...` handle, and an atomic start snapshot; `ok = true` alone is not completion. A fresh unique request must report `created`, while an identical request must report `resumed` with the same ID, including a terminal cached run. Requests with workers `1`, `2`, and omitted must succeed on a local provider with `effectiveWorkers = 1`; a remote provider requested with two workers must retain two. A busy local endpoint must report live `waiting_capacity`, send no overlapping physical request, consume no retry/active budget, and complete automatically without a status-triggered retry. A reasoning-capable local Ollama model with a conflicting configured effort must receive operation-level `reasoningEffort = "none"` and no reasoning reserve. |
| `agentic_map` | A fresh authorized read-only or write-capable request reports `runDisposition = "created"` and returns its durable handle before child provider work begins; `ok = true` or `queued` alone is not success. The run uses a full Kilo child plus the trusted `StructuredOutput` finalizer after any tool work, keeps tagged item/schema content in the user channel, reports `effectiveWorkers`, reaches terminal `completed`, and exposes a valid output handle. The focused full-runtime test must prove child creation, authoritative family registration, read-only denials, provider receipt, structured finalization, durable completion, and output artifact creation through production services. Structured results must cross the child boundary as typed values; fallback text must be unwrapped schema JSON. Compatibility proof must cover one shallow `output` wrapper, one JSON-encoded string only after direct schema failure, preservation of caller schemas that accept wrappers/strings, and typed JSON scalar preservation during final output publication. Diagnostic retries must change the instruction without exposing rejected content. Requests with workers `1`, `2`, and omitted must succeed locally with one effective worker. Foreground, direct-map, and map-child physical calls must remain serialized and alternate when both lanes are queued, without an item-wide reservation. A trivial two-item square map must complete after scheduler shutdown/owning-session activation using persisted parent/agent/directory identity; status polling itself must not resume work. Provider and logical child-slot waiting must appear as `waitingCapacityItems` without advancing active-work time. Typed child/provider errors must retain their classifications; unclassified internal failures use `recovery_failed`, output-length/ignored warning text is not parsed, retryable invalid responses consume only the durable item retry budget, capacity waiting consumes no retry, and cancellation removes logical-slot/provider waiters. |
| `lcm_map_status` | Polling the retained map ID returns `executionState`, terminal `failedItems`, `runningItems`, `waitingCapacityItems`, retryable/capacity counts, `lastProgressAtMs`, durable `lastUpdatedAtMs`, and the latest authorized run counts/output handle without item content. A mixed terminal-failure/capacity snapshot must return the terminal item `safeError` while keeping capacity-based execution state/delay. Creation, runtime recovery, and ordinary owning-session activation schedule unfinished work automatically; status is observational and must not be required to trigger either map kind. A future `retryAfterMs` registers a runtime wake-up without sending duplicate provider work. |
...
GitHub workflow run `30137172326` published prerelease `v7.4.15-lcm.9` as release `359614020` from exact prerelease head `e69b533aa6d18f2a2e4d86919f995c8c9fee366e`, based on product commit `e0037474272a3db361bd6e51288b1a5d655c3dbe`. The workflow overlay guard passed, as did the exact-SHA map, cancellation, tool-contract, sub-agent-scope, and full agentic runtime/provider checks before artifact creation. The packaged Linux x64 runtime smoke passed and uploaded evidence artifact `8613321200`; CLI and VSIX workflow artifacts are `8613280730` and `8613338035`. Independent verification proved that the published prerelease tag resolves to the exact workflow commit, its description names the exact-contiguous grep/provenance and durable-map/runtime fixes, and all 20 required assets—12 CLI archives and 8 platform VSIX files—are nonempty. After replacement verification, superseded release `359305370` and tag `v7.4.15-lcm.8`, bound to `587eaca0fa8e03704078bfc2f398447cf8b241fb`, were deleted by exact release ID, tag, and SHA; after GitHub's short ref-consistency delay both old identities returned `404` while the `.9` release and tag remained present and bound to the expected commit. This completed prerelease automation evidence is not installed-editor or stable-release approval.

GitHub workflow run `30075575942` published prerelease `v7.4.15-lcm.5` as release `359146564` from exact prerelease head `a37433fc0665aa3b1a5406bb40f9e4ff92c37587`. Independent verification proved that the tag resolves to that commit and that all 20 required assets—12 CLI archives and 8 platform VSIX files—are nonempty. The packaged Linux x64 runtime smoke passed and uploaded evidence artifact `8589933348`, proving DB asset loading, both compiled regex-worker paths and cancellation, plus three-process old-session continuation with first-turn `lcm_grep` registration and durable 3/3 then 4/4 Core/LCM coverage. After replacement verification, superseded release `359118859` and tag `v7.4.15-lcm.4`, bound to `15ebfcc659e81cb973c625227e00049ba3b4b5c1`, were deleted by exact release ID, tag, and SHA; remote verification confirmed the old release and tag absent and `.5` still bound to the expected commit with its complete asset profile. Historical `.4`, `.3`, `.2`, `.1`, and `v7.4.13` workflow results are not current-candidate evidence. This completed prerelease automation evidence is not stable-release approval.
