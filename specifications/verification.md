...
GitHub workflow run `30009846221` published prerelease `v7.4.15-lcm.2` as release `358693627` from exact prerelease head `19817f8150c8f78af7ff48d40f6f5e94738d1a06`. Independent verification proved that the tag resolves to that commit and that all 20 required assets—12 CLI archives and 8 platform VSIX files—are nonempty. The packaged Linux x64 runtime smoke passed DB asset loading plus two-process old-session continuation with first-turn `lcm_grep` registration and durable 3/3 then 4/4 Core/LCM coverage. After replacement verification, superseded release `358493732` and tag `v7.4.15-lcm.1`, bound to `90609f2a3fd49564f539ef34d18c86f73c3cc308`, were deleted by exact release ID, tag, and SHA; remote tag verification confirmed the old tag absent and `.2` still bound to the expected commit. Historical `.1` and `v7.4.13` workflow results are not current-candidate evidence. This completed prerelease automation evidence is not stable-release approval.
