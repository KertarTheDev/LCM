...
Transient `provider_capacity_deferred` item failures remain durable `retryable` work, do not consume the item's retry count, publish `retryAfterMs`, and are rescheduled automatically when the runtime provider gate allows work again. `lcm_map_status` does not trigger or resume agentic work. Agentic physical streams use the classified process-wide local-provider lane with background/wait admission, while root, task-child, and explore-child calls use foreground/wait admission. Before prompting, an admitted local agentic item reserves the endpoint for its child session across the complete multi-turn prompt/tool loop. The reservation waits behind the current physical stream, prevents other sessions from taking the endpoint during tool gaps, and still permits only one physical stream at a time. Releasing an item gives an already queued foreground request one provider turn before the next FIFO item reservation. Waiting and active reservations release on cancellation or item cleanup. Per-root/per-workspace child-session slots remain count limits only and do not reject children based on provider endpoint. Classified local providers use the AI SDK middleware path even when experimental native streaming is enabled. Agentic runs start on the next scheduler turn so the invoking parent can register its post-tool continuation first.
...
`lcm_map_status` and `lcm_map_cancel` return content-safe map status. They do not expose item content. `executionState` distinguishes durable capacity backoff from ordinary running state, and `retryableItems`/`capacityDeferredItems` report current retryable rows. `failedItems` counts only terminal item failures. When terminal and capacity-waiting rows coexist, `executionState` and `retryAfterMs` still describe the run-level capacity wait while `safeError` reports the first terminal item error. `lastUpdatedAtMs` changes only on durable run transitions; `retryAfterMs` is a countdown and can decrease between otherwise unchanged snapshots. `retriedItems` remains the number of items whose persisted attempt count exceeds one, so capacity deferrals are excluded. Cancel requests mark runs/items as canceled and release scheduler, provider-wait, or item-reservation work.
