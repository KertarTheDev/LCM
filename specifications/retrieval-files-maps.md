...
Search behavior uses current-lineage candidates and ranks original user/tool/file evidence before assistant discussion, summaries, system/runtime records, degraded summaries, and direct LCM tool echoes. Every hit identifies required `matchKind`, `sourceClass`, `sourceTimestampMs`, and `isLcmToolEcho`, optional `partKind`, `role`, and `toolName`, plus stable source handles. `sourceClass` is the stored provenance class (`user_content`, `assistant_content`, `tool_record`, `system_content`, `runtime_record`, `summary`, or `large_file`); `partKind` is the persisted message-part kind when the hit is part-backed. `isLcmToolEcho` is intentionally narrow: it is true only for a direct persisted `lcm_*`, `llm_map`, or `agentic_map` tool record. It is false for ordinary assistant prose that quotes, interprets, or repeats a prior tool result. Finalized assistant text and tool-call input/output remain searchable, so a lexical hit is not proof that the older source requested by the caller was recovered. Callers must inspect provenance, refine the query when only assistant discussion matches, and follow `page.nextCursor` while `page.hasMore` before concluding that original evidence exists or is absent. Returned snippets are untrusted data and do not grant permission or authorize handles.
...
`lcm_expand_query` answers a focused question using authorized current-lineage memory. It accepts an optional `summaryID`; when supplied, the runtime authorizes that summary in the current lineage and gathers bounded covered source-part and artifact excerpts before the summary prose, query-derived handles, or broad query matches. A stale or out-of-scope optional summary handle is ignored as a bad hint rather than becoming an unhandled tool failure; the broad current-lineage search path can still answer when the query has enough distinctive text. Covered artifact-backed `file_...` handles are preferred so root sessions can recover large tool output when summary prose omitted the handle. Covered original source parts outrank summary text, including for provider-accepted summaries. It also derives literal search candidates from code/path/flag/hash-like spans and stable handles in the question, aggregates matches across all derived queries, globally ranks the bounded candidate set by explicit-handle status, stored provenance, query coverage, lineage, and recency, and calls a retrieval answer generator using prompt version `retrieval-expand-query-v3`.
...
Retrieval answer generation uses `renderLcmPromptRequest(...)`: durable retrieval/citation policy is sent as system content, while the question and authorized excerpts are sent as tagged untrusted user content. Provider-backed generation goes through the centralized LCM provider path used by summaries, file exploration, and direct maps. That path applies provider configuration before operation policy, forwards caller cancellation, and records the effective reasoning policy. Non-agentic generation on a reasoning-capable local Ollama endpoint disables reasoning through the model's `none`/`instant` variant or OpenAI-compatible `reasoningEffort = "none"` override; operation policy wins over a conflicting model default. Extra output reserve is added only when the effective policy can still reason. Unknown adapters are not sent guessed provider-specific flags and report `not_supported` when the requested policy cannot be enforced. Retrieved text cannot grant permissions, authorize IDs, change tool scope, or override instructions, and every memory-derived claim must cite stable handles.

The provider-facing answer format is one fixed structured JSON envelope with required `answer`, `citedHandles`, `coverage`, and `truncated` fields plus optional `confidenceNotes`, `expandedSummaryCount`, and `sourceTokenEstimate`. Local Ollama, native OpenAI, and Azure routes request that fixed schema through the AI SDK structured-output path; other adapters retain prompt-JSON compatibility rather than receiving an unverified schema option. Public tool output keeps the stable answer/citation shape and may include optional `coverage`, `truncated`, `noAnswerReason`, `answerSource`, `fallbackReason`, `providerFailureReason`, `searchedExcerptCount`, `relevantExcerptCount`, `rejectedCitationCount`, and `providerDiagnostics`; raw confidence notes and token estimates are not exposed. Provider diagnostics are content-safe and limited to finish reason, generated text byte count, output token count, reasoning token count, and an empty-text flag. JSON wrapped in Markdown fences or harmless surrounding prose is extracted before validation. JSON-looking malformed output, unsupported citation handles, missing citations, or explicit `coverage = "none"` are provider synthesis failures. Non-JSON prose remains accepted by the legacy normalizer for local/test generators, but production provider instructions and supported native routes require the fixed envelope.

Memory-derived claims must cite stable handles. If no excerpts can support any cited answer, the successful no-answer shape is `ok = true`, `answer = ""`, `citations = []`, `coverage = "none"`, and `truncated = false`, with `noAnswerReason = "no_excerpts"` or `"insufficient_relevance"` as appropriate. A provider failure is reported separately through `providerFailureReason`; it does not turn a weak lexical candidate into evidence. Deterministic extractive fallback is limited to query-relevant explicit-handle excerpts and strongly matched original user/tool/file evidence that matched at least two derived query terms. One-term broad matches, assistant discussion, summaries discovered only through broad search, runtime records, degraded summary metadata, and direct LCM tool echoes are not promoted into fallback claims. When no eligible fallback remains, malformed, empty, declined, or citation-invalid provider output returns the honest empty answer with both `noAnswerReason = "insufficient_relevance"` and the provider failure class. This lets callers distinguish missing/weak evidence from synthesis failure and choose whether to refine grep terms, drop a stale hint, or retry a narrower cited query.
...
Direct `llm_map` generations use the same centralized non-agentic provider policy as summaries, retrieval synthesis, and file exploration. On local Ollama reasoning-capable models this enforces no-reasoning operation options and does not reserve reasoning tokens.
...
The corrected child-runtime/finalizer contract uses agentic output protocol `lcm-map-agentic-runtime-owned-v5` in new request fingerprints. Existing v3/v4 run rows and status remain readable, and unfinished durable work is recovered through its persisted run identity with the corrected current runner. A new otherwise-identical tool invocation intentionally does not deduplicate to a terminal pre-v5 result, which prevents previously canceled/broken terminal runs from masquerading as a fresh successful test. This is an explicit prerelease behavior decision and requires no storage migration.

A valid `StructuredOutput` envelope is unwrapped by the child boundary and returned to the durable map processor as a typed JSON value; it is not canonicalized through assistant text. The trusted finalizer tells a provider that cannot call `StructuredOutput` to emit only the unwrapped schema-conforming JSON value, never the envelope. For compatibility with tool-call-limited local models, a normal stop without a usable finalizer may fall back to the last non-empty assistant text part that is neither ignored nor synthetic. Text compatibility accepts an exact JSON value, one complete JSON/unlabeled Markdown fence, or exactly one balanced object/array within wrapper prose. If that parsed value fails the caller schema, the processor may unwrap exactly one object whose sole key is `output`, and may parse exactly one JSON-encoded string at that boundary; a caller schema that itself accepts the wrapper or string wins and is preserved. Structured values are validated directly, with one exact JSON parse only when the typed value is a string that does not itself satisfy the caller schema. The runtime does not repair malformed JSON, choose between multiple values, recursively unwrap, or reinterpret a schema-valid scalar. Completed JSONB values retain their typed JSON identity during atomic output publication, including strings that contain JSON-looking text.

Each durable retry receives a diagnostic-specific finalization reminder derived from the preceding safe diagnostic, without including the rejected output or item content. Wrapper/schema/JSON failures therefore tell a weak child what contract failed while the durable item retry budget remains the only retry owner. Output-length and unknown-finish states never use fallback text. A terminal child LCM/provider/authentication/content-filter error is propagated before fallback or output validation, so a child LCM memory-limit failure retains its content-safe code and diagnostic instead of being mislabeled as output JSON failure. A terminal `provider_invalid_response` proves that finalization or schema validation failed; it does not by itself prove that the child failed to launch or that the provider model was the only faulty layer.
