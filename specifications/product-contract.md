...
Status: normative v7.7.3 LCM contract.
...
When disabled, Kilo uses the upstream v7.7.3 automatic, provider-overflow, and manual compaction paths, including
...
calls available for reducer/auditor repair, LCM first repairs invalid primary shards with two smaller exact scans
because they otherwise have no local proposal set. A same-size retry is retained when only one repair call remains;
each primary receives at most one repair group. Remaining allowance may replace the earliest
