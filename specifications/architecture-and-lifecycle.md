...
Status: normative v7.7.3 LCM architecture.
...
disabled mode follows the upstream v7.7.3 overflow and compaction contract, including threshold-preflight deferral
...
result to legacy compaction when LCM is disabled. Hidden recovery/finalizer sessions retain that external processor
mode but bypass child-owned LCM capture, projection, and stale-tool payload pruning so their parent-bound research
transcript remains exact; their provider overflow fails the phase and never routes to legacy compaction. Manual
maintenance and LCM tool registration are likewise selected
behind Kilo-owned adapters, so provider requests, queues, interruption, resume state, and legacy compaction retain one
upstream implementation.


## Reuse of Kilo v7.7.3 services

LCM reads raw history through Kilo's session/SQLite services and uses the upstream
`KiloPartLifecycle.transient` classification to exclude UI-only progress from memory,
matching ordinary model-message conversion. Its prior-turn lineage and successful-consumption
rules remain the additional context-management responsibility.

Summary and semantic transformations use the ordinary LLM/provider services and the configured
upstream compaction agent. The Models settings selector owns compaction-model selection; the
Context settings link reuses that surface. LCM does not retain a duplicate model selector.

Tool registration, schema validation, permissions, cancellation, and cost accounting reuse
Kilo services. Private recovery recognizes Kilo's typed `InvalidArgumentsError` and the existing AI SDK error
variants. A narrow compatibility fallback remains for the native LLM decoder's message-only
schema failures; that upstream path does not yet retain a typed error.
Upstream `BoardEnabled` resolves board configuration and runtime flags; locked recovery permissions
keep board/goal/wakeup tools unavailable without replacing the ordinary coordination services.

Recall reuses upstream whole-word, title-typo and partial-match ranking, covering part/message indexes,
and indexed-lookup fallback. LCM adds only active-parent and hidden-session exclusions. The derived
sidecar still owns exact-lineage summaries and consumption because recall does not provide those.
