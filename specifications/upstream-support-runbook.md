...
## Support And Packaged-Runtime Commands

DB diagnosis, stale-lock recovery, and rebuild are runtime-owned session routes. They derive the family from a trusted session ID and never accept a caller-supplied data directory. Recovery/rebuild default to dry-run and reject unknown payload fields.
...
The packaged command must execute `debug lcm-db-smoke` through that extracted binary. It must also execute `debug lcm-session-continuation-smoke` as one seed process followed by two continuation processes against the same session. This proves that old-session continuation uses the bundled Core database migrations and one durable LCM conversation across restarts, rather than only proving a fresh PGlite open. The regex check proves both the PGlite cancellation worker and the isolated retrieval-regex worker resolved through their compiled paths. A compile-only result, source-tree smoke, v1 evidence document, or single-process continuation does not prove the VSIX contains and reopens those entrypoints correctly.
