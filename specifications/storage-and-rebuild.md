...
Schema version 11 contains session lineage and durable `consumed_through`, bounded source metadata/excerpts, immutable
summary nodes/edges/attempts, frontier revisions, activity, normalized context frames/blobs, and leases. Tree policy
`lcm-tree-v8` is part of summary structural identity. Version 11 deliberately rebuilds version-10 sidecars so retained
summaries cannot preserve unrelated transformation output or embedded protocol scaffolding accepted before grounding,
receipt omission, and authoritative handle allowlists. It also binds recovery output to deterministic-call counters
and explicit transport-source chronology/semantic-boundary guidance. The disposable sidecar rebuild does not modify
Kilo SQLite.
