...
Status: normative v7.7.3 storage contract.
...
Tool-output offloading and its seven-day file retention remain upstream-owned. Conversation Memory does not archive
offloaded files or import their full bodies into the derived store. When an offloaded output is missing, use the
information retained in the summary tree; uncaptured details may need to be regenerated. Missing offloaded files do
not invalidate otherwise valid summaries. Exact-byte citations still require retained source bytes.
...
The active frontier is the most recently published revision for the current lineage, ordered by SQLite insertion
order rather than timestamps or random ID suffixes. Equal or regressing timestamps cannot reactivate an older
frontier. An additive session/lineage index supports this lookup without invalidating existing version-14 sidecars.
