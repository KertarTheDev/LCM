...
- Continuation-focused summaries preserve intent, permissions, decisions, exact working details,
  verified progress, and unresolved work, making long-session handoffs more useful.
- Recovery grep supplies UTF-8-safe copy-ready citation ranges so isolated workers do not have to
  calculate byte offsets. Semantic recovery also normalizes complete structural scopes and preserves
  verified events through bounded shard repair; remaining ordering uncertainty stays visible.
- Hard maintenance measures the complete rendered request and continues reducing eligible history
  while progress is possible. Irreducible requests still fail closed. Frontier publication order no
  longer depends on timestamps or random identifiers.
- The new Kilo integration reuses native transient-part classification, typed tool-validation errors,
  board configuration, recall indexes/ranking, and the Models compaction selector. Native goals,
  wakeups, provider requests, permissions, and cancellation retain upstream ownership.

The latest focused 100k Qwen-alias diagnostic recovered an omitted venue reference with one child
and one grep, including a verified 512-byte citation; the retained legacy observation was incorrect.
All-in logical input was 1,342,560 versus 1,639,590, but LCM used more fresh input and output, so this
is not a universal cost-saving claim. A 32k stress retry succeeded while many summary attempts hit
the output limit; semantic ordering and natural-workflow generality remain limitations. No full
benchmark or new-model result is claimed for this port.
