...
`provider-capacity.ts` classifies provider endpoints and gates all LCM-active local Ollama/OpenAI-compatible provider streams in one process-wide lane per endpoint. Root calls and trusted task-child or explore-child calls use foreground/wait admission. Agentic map physical streams use background/wait admission. Capacity is acquired per physical model turn, not per agentic item. When foreground and map-child turns are both queued, the lane alternates them while preserving FIFO order inside each priority, preventing either the parent or a multi-turn child from monopolizing a one-request local provider. Durable background maintenance keeps background/defer admission and can return `provider_capacity_deferred` with safe hashed endpoint identity, capacity class, retryable guidance, and action `retry`.

The AI SDK path acquires capacity before provider invocation, outside the response-idle watchdog, and releases it exactly once when the physical stream completes, errors, or is canceled. The map child phase bridge persists `waiting_capacity` before queueing and restores `running` on admission; capacity-wait time is excluded from the item's bounded active-attempt clock. A local background child uses a longer first-event window only when the provider has no explicit chunk-timeout setting, while subsequent event gaps retain the normal watchdog. Map children disable provider-internal retries so the durable item retry budget is the single retry owner. Child-session admission slots continue to enforce per-root and per-workspace counts, but do not duplicate provider arbitration or reject two children merely because they select the same local endpoint. Remote/unknown providers retain their normal concurrency and explicit timeout behavior.
