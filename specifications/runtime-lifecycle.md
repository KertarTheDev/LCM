...
`lcmActive` is true only for `lcm_active`. Product prompt ownership is nevertheless fixed to LCM at construction time: both `lcm_active` and `passive_synced` use the full persisted message stream before preflight finishes activation, while unavailable, recovery, or unsupported states fail before a normal provider request. They do not select legacy compacted-history filtering. Persisted legacy `compaction` parts remain source compatibility data and are removed only from the prompt control projection so an old marker cannot re-enter the obsolete compaction control flow. A proven `passive_synced` scope may register its capability-scoped LCM tool schemas only into the local preflight candidate payload so the first successful continuation turn already has retrieval tools; that payload is not sent until provider-safe assembly succeeds and the conversation is marked `lcm_active`. Tool execution still requires the active lifecycle state plus valid boundary metadata and valid capability metadata. Malformed child capability metadata makes tool registration and retrieval unavailable even if a conversation row exists.
...
Production family resolution, lifecycle message counting, session/project lookup, finalized-source loading, and session-scoped settings resolution read from the same Core Kilo database service that owns current sessions. `LcmRuntime.layer` captures that authoritative service when the runtime is constructed and explicitly supplies it to lifecycle/family subeffects, including agentic-map child registration and queued prompt loops whose ambient Effect context may differ from the initiating call. App-runtime callers therefore cannot silently fall back to a separately materialized Core database while registering a freshly created child. The standalone LCM default layer installs the service explicitly, and isolated Effect embeddings that omit the service acquire `CoreDatabase.defaultLayer` inside their own scope, so no LCM module imports the legacy `@/storage/db` singleton or creates a separate module-owned database runtime. Family resolution validates each parent stage, project/workspace scope, canonical child/parent directories, project worktree/sandbox boundaries, cycle/self links, and the derived family root. Failures retain stage-specific diagnostics such as session lookup, parent lookup, project boundary, data-directory, or family-root resolution; the former catch-all `lcm_family_resolution_failed` is not a normal product diagnostic. Standalone direct helpers remain only for isolated DB/layout tests.

## Prompt-Time Preflight

The normal provider path in `session/prompt.ts` initializes or resumes LCM after the assistant turn row exists, loads the runtime-proven conversation scope, prepares a local preflight candidate with that scope's persisted capability class, and calls `preflightBeforeModel(...)` before any provider work. Root, task, explore, and agentic-map child prompts therefore present the same capability class during render preparation and provider assembly that lifecycle proof assigned to their conversation; the prompt adapter does not hard-code child requests as root. For an existing `passive_synced` session, candidate render preparation may include only the tools authorized by its proven boundary/capability scope; runtime execution remains active-only. Preflight then synchronizes finalized source, proves rebuildable provider-safe assembly, marks the conversation `lcm_active`, and only then sends the already scoped payload to the model. An initialization, scope, or activation error or defect is normalized to `LcmSafeError`, persisted on a zero-token assistant as `LcmMemoryError`, and closes the turn with `finish = "error"`; it does not escape as `UnknownError` or select legacy compaction. If prompt-time render preparation returns `LcmSafeError`, the assistant message is completed with a content-safe `LcmMemoryError`, the session error event is published, and no provider request is sent.
...
Ordinary parent prompt cancellation and recursive task cleanup do not cancel a descendant subtree whose root is proven by the runtime family DB to be a `map_child`. Durable map scheduling, explicit `lcm_map_cancel`, session deletion, and runtime shutdown own that work independently of the initiating prompt. The exclusion is fail-closed: a missing/failed proof does not skip an ordinary child, and directly canceling a map-child session still cancels that child and its descendants.
...
`provider-capacity.ts` classifies provider endpoints and gates all LCM-active local Ollama/OpenAI-compatible provider streams in one process-wide lane per endpoint. Root calls and trusted task-child or explore-child calls use foreground/wait admission. Direct `llm_map` calls and agentic map physical turns use background/wait admission. Capacity is acquired per physical model call, not per map item. When foreground and map turns are both queued, the lane alternates them while preserving FIFO order inside each priority, preventing either side from monopolizing a one-request local provider. Durable background maintenance keeps background/defer admission and can return `provider_capacity_deferred` with safe hashed endpoint identity, capacity class, retryable guidance, and action `retry`.

The AI SDK and direct generation paths acquire capacity before provider invocation, outside the response-idle watchdog, and release it exactly once when the physical response completes, errors, or is canceled. Both map paths persist `waiting_capacity` before queueing and restore `running` on admission; capacity-wait time is excluded from attempts, retries, and the bounded active-attempt clock. A local background child uses a longer first-event window only when the provider has no explicit chunk-timeout setting, while subsequent event gaps retain the normal watchdog. Map children disable provider-internal retries so the durable item retry budget is the single retry owner. Child-session admission slots still enforce per-root and per-workspace counts; map children wait abortably for those logical slots, while ordinary task/explore callers retain the existing immediate limit error. Remote/unknown providers retain their normal concurrency and explicit timeout behavior.

The runtime records endpoint/capacity metadata without raw URLs. A typed map execution plan separates requested worker maximum from persisted effective workers and capacity class. Local endpoints, small-model selectors, or observed foreground pressure may lower effective workers before durable run creation; remote/cloud providers retain valid requested/default concurrency.
