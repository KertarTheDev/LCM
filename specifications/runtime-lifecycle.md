...
The normal provider path in `session/prompt.ts` first activates LCM after the assistant turn row exists but before render preparation, tools, or provider work. An activation error or defect is normalized to `LcmSafeError`, persisted on a zero-token assistant as `LcmMemoryError`, and closes the turn with `finish = "error"`; it does not escape as `UnknownError` or select legacy compaction. The path then prepares prompt render inputs, validates active render-only metadata for LCM-active sessions, and calls `preflightBeforeModel(...)` before streaming to the model. If prompt-time render preparation returns `LcmSafeError`, the assistant message is completed with a content-safe `LcmMemoryError`, the session error event is published, and no provider request is sent.
