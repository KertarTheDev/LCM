...
Session DB lock recovery is a guided runtime-owned support action. `recoverDbLock(...)` derives the family root from trusted session lineage and defaults to dry-run preview. Apply mode closes the runtime-owned family worker before mutation when present, quarantines only `owner.lock`, refuses fresh/live-owner locks, requires explicit `force` for malformed or otherwise uncheckable locks, reopens the existing family DB, and reports content-safe status plus safe errors only. It is not a full DB rebuild and does not quarantine `pglite/`, delete memory rows, or inspect raw memory.
