...
Production family resolution reads session, project, workspace, and parent lineage from the same Core Kilo database service that owns current sessions. App-runtime callers use the ambient Core service and the standalone LCM default layer installs it explicitly. Isolated Effect embeddings that omit the service acquire `CoreDatabase.defaultLayer` inside their own scope, so the resolver does not leak a new layer requirement or create a separate module-owned database runtime. It validates each parent stage, project/workspace scope, canonical child/parent directories, project worktree/sandbox boundaries, cycle/self links, and the derived family root. Failures retain stage-specific diagnostics such as session lookup, parent lookup, project boundary, data-directory, or family-root resolution; the former catch-all `lcm_family_resolution_failed` is not a normal product diagnostic. Standalone direct helpers remain only for isolated DB/layout tests.
