...
`provider-capacity.ts` classifies provider endpoints and gates all LCM-active local Ollama/OpenAI-compatible provider streams in one process-wide lane per endpoint. Root calls and trusted task-child or explore-child calls use foreground/wait admission. Agentic map physical streams use background/wait admission. Before prompting, an admitted agentic item acquires an exclusive endpoint reservation keyed to its child session; the reservation waits for any active physical stream, then lets only that child session use the endpoint until the complete item prompt/tool loop finishes. The physical stream count remains one. A queued item reservation receives the next opportunity after the current stream, while releasing an item reservation gives an already queued foreground request one opportunity before the next reservation. Multiple item reservations remain FIFO. Durable background maintenance keeps background/defer admission and can return `provider_capacity_deferred` with safe hashed endpoint identity, capacity class, retryable guidance, and action `retry`.

The AI SDK middleware acquires capacity immediately before each physical provider stream starts and releases it exactly once when that stream completes, errors, or is canceled. Ordinary calls do not retain capacity across client-side tool execution; the separate agentic-item reservation retains endpoint ownership across those gaps without pretending a physical stream is active. Waiting or active reservations are released on item cancellation and in the child runner's completion/failure cleanup. Child-session admission slots continue to enforce per-root and per-workspace counts, but do not duplicate provider arbitration or reject two children merely because they select the same local endpoint. Classified local calls fall back from the experimental native runtime to this controlled path; remote native calls remain unchanged.
