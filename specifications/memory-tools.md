...
UTF-8 offsets. Repeated matching labels may be narrowed by one unambiguous natural ordinal attached to the normalized
boundary label (`episode 3`, `episode three`, or `third episode`) or by an ordinal qualifying the exact opening marker.
Comparisons, ranges, and multiple scoped unit mentions retain the complete matching set instead of guessing one unit.
The prefetch then selects only from the matching raw boundary envelope; overlapping summary labels and claims cannot
redefine that scope. The main session still receives none of this map or raw evidence. Retrieval uses
...
trusted question that selects exactly one structural unit, a child scope that fully covers the unit is conservatively
narrowed to the exact marker-interior content scope. This tolerates omitted endpoint offsets or harmless surrounding
records without admitting their bytes into inference; a clipped scope that omits any unit byte or a question matching
several units is not upgraded. For a generated range that does not match a complete trusted structural unit, child
guidance distinguishes retrieved-range coverage from exact-unit coverage. It directs the child to reuse completed unit
analysis or copy the existing
...
question. Before reduction, LCM prioritizes invalid primary shards with two smaller exact repair scans when spare
allowance permits, retaining a same-size retry only when one repair call remains. It may then atomically claim
...
If the combined exact envelope exceeds the range limit but every unit remains represented in the bounded map,
coverage is tracked per non-empty unit and can be completed through separate exact-unit analyses. The missing
combined envelope alone is not a permanent coverage gap. A map that omits units remains incomplete.
Literal structural-label matching uses complete normalized words and numbers with the same separator normalization
as boundary labels: a query for `window 10` does not exactly match `window 1`. Literal label numbers do not imply a
chronological position among repeated unnumbered markers.
...
ranges for a source-scoped search) without duplicating that preview at the record level. Every occurrence reports the
preview's exact `excerptByteRange`; a raw-source occurrence also supplies the same interval as a copy-ready `citation`
object, bounded to 512 UTF-8 bytes, so the child never has to calculate offsets from displayed text. Results also include
an exact occurrence-page offset/total/next offset, `rangesComplete`, `occurrencesComplete`,
...
exact source byte lineage and its complete text. Local shards collect qualifying predecessors and possible targets;
the requested unit-level ordinal is a later selection operation, not a local event filter. A shard with fewer than
the requested number of events returns its supported events as partial rather than treating the whole question as
unanswerable there. Asking who acted requests an output attribute; only an explicit actor restriction filters events
by actor. The reducer sees bounded result JSON from every successful
...
proposals that fail those criteria before ordering. Event status is question-defined: plans, mentions, rejected
attempts, hypotheticals, and continuations remain eligible when requested, but cannot prove a newly completed action.
Neither a shard, reducer, nor auditor may impose a universal completed-action filter. If one shard has no valid
...
calls available for pass repair, invalid primary shards receive two smaller exact chronological repair scans before
optional semantic subdivision of other uncertainty. If only one repair call remains, a same-size retry is retained.
Each primary shard receives at most one repair group. Remaining allowance can split the earliest unconfirmed-negative or partial positive primary
...
host rejection category and the requested event rank plus verified event count, never the rejected response, and undergoes
identical validation. Rejection distinguishes missing event records, records with no independently byte-verified events,
and too few distinct verified events to establish the requested ordinal. Repair requires the complete prefix or suffix
through the target and explicitly preserves uncertainty rather than inventing events to meet the count. Bounded host-only
