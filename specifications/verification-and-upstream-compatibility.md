...
Status: normative v7.7.3 acceptance contract.
...
- v7.7.3 shared-board registration and notices remain available to ordinary agents but unavailable to both locked
...
- disabled mode matches upstream v7.7.3 automatic, provider-overflow, `compaction.auto=false`, threshold, current-tool
...
- unchanged v7.7.3 remote, Project Memory, structured output, request identity, skill, first-response-byte timeout,
...
- per-user editor context and its creation time remain protected through final projection for provider-cache stability,
  ...
  task results retain the v7.7.3 lifecycle;
- the v7.7.3 prompt tool-service environment, GPT-5.6 OAuth capacity, raw SQLite WAL recovery, Ask-to-Code reminder,
  and TUI created-time chronology remain upstream-owned and reach LCM through the ordinary host paths; and
- the v7.7.3 read-only permission baseline exposes only `lcm_query` to ordinary agents while explicit user denies still
