...
| Local-provider deferred background capacity | Durable background LCM maintenance for a local Ollama or local OpenAI-compatible endpoint would overlap an active provider stream, queued foreground work, or an agentic-item endpoint reservation | `provider_capacity_deferred` | `lcm.provider_capacity.deferred` | `retry` | `true` | `LcmMaintenanceResult.status = "deferred"` and/or content-safe event/metrics diagnostics. No provider request is sent. Root/task/explore streams wait in the foreground lane. Agentic map-child physical streams use background/wait admission under an exclusive child-session reservation that spans the whole item and preserves one physical stream at a time. Post-invocation timeout/capacity failures use their closest safe code, and connection failures use `provider_unavailable`, not `provider_capacity_deferred`. |
...
`LcmMapResult.retriedItems` is the count of distinct map items whose persisted `attempts` value is greater than `1` in the returned snapshot. It is not total retry attempts and not the count of currently `retryable` items. `retryableItems` is the current retryable-row count, `capacityDeferredItems` is the subset carrying `provider_capacity_deferred`, and `failedItems` is the terminal failed-row count. `lastUpdatedAtMs` is the durable run timestamp and changes only when run state is persisted; `retryAfterMs` is computed from the current clock and can count down without changing that timestamp. `executionState = "waiting_capacity"` is computed when a nonterminal run is in durable capacity backoff; otherwise it mirrors queued/running/terminal execution. If terminal failed rows coexist with capacity-deferred rows, the status `safeError` is the first terminal item error while execution state and retry delay remain run-capacity based. Item rows still preserve total attempts for retry/lease logic.

Map tools are durable asynchronous run tools. Initial `llm_map` and `agentic_map` calls do not accept `mapID`; they validate/register input, create or resume the durable run, enqueue/claim work, and return the latest `LcmMapResult` snapshot without requiring all items to complete during the initial tool call. A new agentic run defers its first scheduler turn until after the durable handle is returned so the invoking parent continuation can register its local-provider request. For a classified local endpoint, the admitted item then owns the endpoint across its complete child prompt/tool loop; a queued foreground request gets one opportunity when that reservation releases and before the next item reservation. Physical provider concurrency remains one. If a run finishes before its initial call returns, the snapshot may be `completed`; otherwise callers retain its `mapID` and use `lcm_map_status` to poll no sooner than `retryAfterMs`. Status may resume eligible `llm_map` work but only reports automatically scheduled agentic work. `lcm_map_cancel` requests cancellation and returns the latest content-safe snapshot. `outputFileID` appears only after the run reaches `completed`.
