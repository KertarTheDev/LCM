...
When `LcmPageInput.limit` is omitted, tools apply the default page limit from `RUNTIME_DEFAULTS.retrieval` in `packages/opencode/src/session/lcm/config.ts` and echo the applied value in `LcmPageInfo.limit`. Paged retrieval tools must reject values above the canonical maximum before reading content, metadata, filesystem, or provenance data and return a content-safe over-limit error. They must not silently clamp over-limit requests; `LcmPageInfo.limit` reports the applied limit only for accepted requests.
...
| Map known run | Authorized run fails or is canceled after a durable snapshot exists | `timeout`, `canceled`, `over_limit`, `provider_unavailable`, `provider_invalid_response`, `recovery_failed`, or closest safe code in `safeError` | Matching template | By code | By code | `LcmMapResult` with `status = "failed"` or `"canceled"` and optional schema-validated, normalized `safeError`; `invalid_request` is reserved for pre-run request validation. Malformed persisted map safe-error JSON is omitted or replaced by a fallback instead of surfaced. |
...
| Local-provider deferred background capacity | Durable background LCM maintenance for a local Ollama or local OpenAI-compatible endpoint would overlap an active or queued provider turn | `provider_capacity_deferred` | `lcm.provider_capacity.deferred` | `retry` | `true` | `LcmMaintenanceResult.status = "deferred"` and/or content-safe event/metrics diagnostics. No provider request is sent. Root/task/explore streams wait in the foreground lane. Both map kinds use background/wait admission in the same turn-level fair lane, so ordinary map contention is `waiting_capacity`, not a safe error or retry. Agentic children do not reserve capacity across tool work. Post-invocation timeout/capacity failures use their closest safe code, and connection failures use `provider_unavailable`, not `provider_capacity_deferred`. |
...
For current maintenance paths, `maintenanceInputBudget` is the authoritative source-selection budget after maintenance prompt/wrapper overhead has been subtracted once. `maxSourceTokens` remains an archived compatibility alias for older helpers only. Legacy callers may normalize `maxSourceTokens` into `maintenanceInputBudget` before constructing a request, but current helpers must reject any request that provides both fields, even when the finite values are equal, before maintenance starts.
...
All runtime-owned non-agentic LCM generation—maintenance summaries, `lcm_expand_query`, file exploration, and direct `llm_map` items—uses one centralized provider policy after normal provider/model configuration is merged. For a reasoning-capable local Ollama route, the effective policy is `no_reasoning`; a model `none`/`instant` variant is preferred and OpenAI-compatible Ollama otherwise receives operation-level `reasoningEffort = "none"`. This operation override wins over a conflicting configured default, and reasoning output reserve is omitted. A non-reasoning model needs no provider-specific override. Unknown/remote adapters retain provider defaults unless the requested policy maps to a declared model variant; the runtime must not guess unsupported provider option keys and records `not_supported` when enforcement is unavailable. Agentic-map child reasoning remains owned by the normal child-session provider path and is not silently rewritten by this non-agentic policy.
...
interface LcmGrepResult {
  ...
  scopeWarning?: "summary_not_found" | "summary_outside_scope" | "summary_invalid"
  results: Array<{
...
type LcmExpandQueryProviderFailureReason =
  | "provider_empty"
  | "provider_malformed_json"
  | "provider_citation_rejected"
  | "provider_declined"
...
Every `lcm_grep` hit includes `sourceClass`; part-backed hits also include `partKind`. `isLcmToolEcho` is true only for a direct persisted LCM/map tool record and remains false for assistant prose that quotes or discusses prior tool output. Search ranking places original user/tool/file evidence before assistant discussion, summaries, system/runtime records, degraded summaries, and direct LCM tool echoes. These fields describe stored provenance; a lexical hit is not proof that the requested older source was recovered.

Committed deterministic fallback summaries are degraded memory. Retrieval preserves them as authorized summary handles, but `lcm_grep` ranks matching original message parts ahead of matching fallback summaries in the same lineage. Summary-shaped `lcm_grep`, `lcm_describe`, and `lcm_expand` results include `summaryDegraded = true`, `summaryObjectiveStatus = "fallback_accepted"`, and the stored fallback mode, currently `summaryFallbackMode = "extractive_key_points"` for new fallback rows and `"truncated_prefix"` for older compatible rows. `lcm_expand_query` resolves an explicit summary handle supplied through `summaryID` to covered source-part and artifact excerpts before summary prose when those rows are still authorized, regardless of whether the summary was provider-accepted or deterministic fallback.
...
Retrieval pagination, `lcm_grep` pattern limits, result byte caps, `lcm_expand_query` answer defaults, and `lcm_read` byte defaults are defined canonically by `RUNTIME_DEFAULTS.retrieval` in `packages/opencode/src/session/lcm/config.ts`. Tool input validation must apply those limits before content or metadata is read.
...
`lcm_expand_query` no-answer is a successful empty answer, not a synthetic uncited claim: return `LcmExpandQueryResult` with `ok = true`, `answer = ""`, `citations = []`, `coverage = "none"`, and `truncated = false` when authorized retrieval results cannot support an answer with stable `sum_...`, `file_...`, `msg_...`, or `part_...` citations. `noAnswerReason = "no_excerpts"` means no authorized excerpt was found; `"insufficient_relevance"` means candidates existed but none qualified for deterministic fallback. `providerFailureReason` separately reports empty, malformed, declined, or citation-invalid provider synthesis and must not replace the evidence reason. `searchedExcerptCount` reports distinct candidates considered and `relevantExcerptCount` reports the bounded candidates sent to synthesis.

Provider failure returns a bounded deterministic extractive fallback only from query-relevant explicit-handle excerpts or strongly matched original user/tool/file evidence that matched at least two independently derived query terms. One-term broad matches, assistant discussion, summaries discovered only through broad search, system/runtime records, degraded summary metadata, and direct LCM tool echoes are not promoted into fallback claims. If no eligible fallback remains, return the successful empty answer with `noAnswerReason = "insufficient_relevance"` and `providerFailureReason`; do not concatenate arbitrary search hits. When `summaryID` is present, the runtime authorizes that summary in the current lineage and loads covered source-part/artifact excerpts before summary prose and query-derived matches. When the query text explicitly names authorized `file_...` handles, the runtime may include bounded UTF-8 excerpts from LCM-owned artifact-backed file rows, including `tool_output` artifacts, after artifact validation. Framed inline-part artifacts are decoded into logical text/reasoning/tool input/output/error/media sections before becoming model-visible excerpts; raw frame headers and length fields are not exposed. Path-backed file rows remain preview-only through `lcm_expand_query` and require authorized `lcm_read` for exact bytes.

Provider-backed `lcm_expand_query` uses prompt version `retrieval-expand-query-v3`. The provider is asked for one fixed JSON envelope with `answer`, `citedHandles`, `coverage`, `truncated`, and optional diagnostic fields such as `confidenceNotes`, `expandedSummaryCount`, and `sourceTokenEstimate`. Local Ollama, native OpenAI, and Azure use the AI SDK native structured-output request for this fixed trusted schema; other adapters use prompt-JSON compatibility. Public output preserves `answer` plus typed `citations` and optional content-safe diagnostics. Provider-backed output may include `providerDiagnostics` containing finish reason, generated text byte count, output token count, reasoning token count, and whether the provider returned empty text; it must not include prompts, excerpts, raw provider text, raw request bodies, or provider-specific payloads. JSON wrapped in Markdown fences or harmless surrounding prose may be extracted before schema validation. Non-JSON legacy prose is normalized by the older citation extractor for local/test generators, but production prompt instructions require the structured envelope. `confidenceNotes`, expanded summary counts, and source token estimates remain parser inputs only unless this contract is deliberately expanded later.
...
`storageWarningThresholdBytes` is the effective configured threshold. The default comes from `PUBLIC_DEFAULTS.storage` in `packages/opencode/src/session/lcm/config.ts`, but lower finite positive values are valid for quota-sensitive environments. `storageWarning` is true when `storageBytes >= storageWarningThresholdBytes`; settings and metrics must not introduce automatic cleanup, LCM-only deletion, or raw memory inspection. The internal `freshTailTokens` threshold/metrics field is deliberately absent from both settings DTOs.

Settings updates resolve scope through `sessionID`, `workspaceID`, and `projectID` using `settings-state.ts`: a trusted session-derived scope takes precedence, then an explicit trusted workspace, then project, then defaults. In v1, `sessionID` identifies the current workspace/project scope and does not create a persisted per-session settings row. `GET /lcm/settings` and `PATCH /lcm/settings` are the primary settings routes and must work before a chat session exists; they derive the authorized project/workspace from trusted runtime request context, using the current workspace only when present. `GET /lcm/settings` accepts optional `projectID` and `workspaceID` assertion query parameters; `PATCH /lcm/settings` accepts optional `projectID` and `workspaceID` assertions in `LcmUpdateSettingsInput`. Client-supplied IDs are assertions, not authority; if present, they must match the trusted request context or the runtime returns `LcmSafeError` with `code = "invalid_request"` before reading or writing. `GET /session/:sessionID/lcm/settings` and `PATCH /session/:sessionID/lcm/settings` derive scope from the trusted path `sessionID` and may attach runtime-owned `lifecycleState`, `dbStatus`, and capability `safeError` diagnostics for that session/family. A session-route settings write must reject a body `sessionID` that differs from the path `sessionID`; if the body also provides `projectID` or `workspaceID`, those explicit IDs must match the scope derived from the session or the runtime returns `LcmSafeError` with `code = "invalid_request"` before writing. Only `strategy` and `storageWarningThresholdBytes` are writable and persisted. `freshTailTokens` remains an internal threshold/metrics field and is absent from settings state; storage usage, lifecycle, family DB status, and cost fields are derived runtime state. The two public settings persist through normal Kilo settings/config storage at workspace scope when a workspace is available, otherwise project scope; built-in defaults and valid Kilo config deployment defaults are reported with `effectiveScope.kind = "default"` when no persisted override exists. The runtime config write shape is exact: `strategy` becomes `lcm.strategy` and `storageWarningThresholdBytes` becomes `lcm.storage.warningThresholdBytes`; no other public `lcm.*` config keys are written by settings routes.
...
Prompt and command dispatch failures continue to use the normal extension-to-webview `sendMessageFailed` message so the composer draft and file attachments are restored through the existing prompt input recovery path. The v7.4.15 extension host does not prewarm or own LCM storage; readiness, retry, and owner-lock handling remain runtime responsibilities behind the generated SDK. If prompt or command dispatch fails with a structured runtime `LcmSafeError`, the payload may include optional `safeError?: LcmSafeError` preserving the runtime code, retryability, action, safe message, and diagnostic code. The webview must prefer `safeError.safeMessage` for user-facing memory recovery copy and must not expose raw DB paths or memory content.
...
type LcmMapExecutionState = "queued" | "running" | "waiting_capacity" | "completed" | "failed" | "canceled"
...
type LcmMapStartDisposition = "created" | "resumed"

interface LcmMapStartResult extends LcmMapResult {
...
Map inputs must provide exactly one of `inputFileID`, `inputPath`, or `inputJsonl`. `inputJsonl` is registered as an LCM-owned `map_input` artifact before item rows are created. `inputPath` and path-backed `inputFileID` values pass normal provenance/permission checks and are then copied into an immutable `map_input` artifact; the run's `inputFileID` always identifies that durable snapshot. JSONL input must be UTF-8, may have one leading UTF-8 BOM, must not contain empty or whitespace-only lines, and must parse to exactly one JSON value per physical line. Item indexes are zero-based in physical line order after BOM handling. Completed output JSONL is one schema-valid JSON value per input item in ascending item-index order, with no wrapper object and no missing completed items. `maxRetries` counts retries after the initial provider/response attempt, defaults to `2`, must be a non-negative integer, and is capped at `5` by `RUNTIME_DEFAULTS.map` in `packages/opencode/src/session/lcm/config.ts`. Transient provider-capacity waiting or deferral is automatic and consumes neither this retry budget nor `retriedItems`.
...
`LcmMapResult.retriedItems` is the count of distinct map items whose persisted `attempts` value is greater than `1` in the returned snapshot. It is not total retry attempts and not the count of currently `retryable` items. `retryableItems` is the current retryable-row count, `capacityDeferredItems` is the subset carrying durable `provider_capacity_deferred`, `runningItems` is claimed active work outside provider waiting, `waitingCapacityItems` includes live `waiting_capacity` phases plus durable capacity deferrals, and `failedItems` is the terminal failed-row count. `lastProgressAtMs` records the latest useful durable map transition; `lastUpdatedAtMs` records any durable run transition. `retryAfterMs` is computed from the current clock and can count down without changing either timestamp. `executionState = "waiting_capacity"` is computed when a nonterminal run has live or durable capacity waiting; otherwise it mirrors queued/running/terminal execution. If terminal failed rows coexist with capacity-deferred rows, the status `safeError` is the first terminal item error while execution state and retry delay remain run-capacity based. Item rows still preserve total attempts for retry/lease logic.

Map tools are durable asynchronous run tools. Initial `llm_map` and `agentic_map` calls do not accept `mapID`; they validate/register immutable input, atomically create or resolve the durable run, schedule work, and return `LcmMapStartResult` without requiring all items to complete during the initial tool call. `runDisposition = "created"` means this invocation inserted the run; `"resumed"` means deterministic identity resolved an existing run, including an older terminal run. The start snapshot is the atomic create/resume result and is not a post-schedule status poll. Input preparation remains caller-cancelable, but once durable creation begins the create-and-schedule boundary is uninterruptible; a caller abort before durable creation returns `lcm_map_aborted_before_durable_create` and an abort after durable creation does not leak into the independent scheduler child signal.

A classified local endpoint serializes physical provider calls and alternates foreground and background map turns when both lanes are queued; agentic children release capacity between model turns and tool work. Remote/unknown providers retain normal concurrency. Scheduling is automatic at creation, successful owning-session activation, and runtime recovery. `lcm_map_status` is strictly observational and does not ensure scheduler registration, resume a run, or invoke provider work. A future retry delay is registered by the scheduler as a runtime wake-up without invoking the provider early. `lcm_map_cancel` requests cancellation and returns the latest content-safe snapshot. `outputFileID` appears only after the run reaches `completed`.

Runtime map worker count is an effective execution parameter, not only a literal tool input echo. `workers` is a requested maximum; a typed internal execution plan carries it separately from `effectiveWorkers` and provider capacity class. The runtime may lower a valid requested/default value before durable run creation for a local endpoint, active/queued foreground work, or the `small` model selector. Valid local values of one, two, or omitted workers produce a run rather than a worker-count error. Remote/cloud providers retain requested/default concurrency. The effective count remains part of durable create/resume identity and is returned as `LcmMapResult.effectiveWorkers`.

Create/resume identity is runtime-owned. When Kilo provides a durable tool-call ID for the map tool invocation, the runtime resumes by `(conversationID, toolKind, sourceToolCallID)`. When no durable tool-call ID exists, `map.ts` derives the canonical deterministic `requestFingerprint` from tool kind, authorized conversation, immutable input artifact ID and hash, prompt hash, canonical model selection, schema hash, agentic mode, worker count, and retry count. `agentic_map` additionally persists parent session, submitting agent, parent directory, and provider capacity class and versions its runtime-owned structured-output protocol in the fingerprint. Protocol `lcm-map-agentic-runtime-owned-v5` rotates that fingerprint after the provenance-aware finalizer repair: unfinished v3/v4 rows remain recoverable through persisted runtime identity, but a new invocation does not deduplicate to an old terminal pre-v5 failure. This lets runtime recovery or successful owning-session activation reconstruct full Kilo child execution after process restart without retaining the original tool closure. A matching resume candidate whose stored prompt, model selection, schema, mode, input file, worker count, or retry settings conflict with the current request returns `LcmToolErrorResult` with `invalid_request` before reading item content.
...
Valid structured output takes precedence over assistant text and crosses the child boundary as a typed JSON value after the trusted one-property envelope is removed. If a provider cannot call the finalizer, its system instruction requires only the unwrapped schema-conforming JSON value as assistant text. On a normal stop without a usable finalizer, compatibility parsing considers only the last non-empty assistant text part that is neither ignored nor synthetic. Parsing accepts a complete JSON value, one complete JSON/unlabeled Markdown fence, or exactly one balanced object/array embedded in wrapper prose. If the parsed text fails the caller schema, the runtime may unwrap exactly one object whose sole property is `output`, and may parse one JSON-encoded string at that boundary. A caller schema that directly accepts the wrapper or string wins; recursive unwrapping and reinterpretation of schema-valid scalars are forbidden. Structured string values receive one exact JSON parse only when the string itself fails the caller schema. AJV validates every accepted value, and completed JSONB values retain their typed JSON identity during output publication.

Output-length, unknown-finish, missing finalizer, empty/malformed/ambiguous output, wrapper/schema failures, and schema failures use retryable `provider_invalid_response` errors with stable diagnostics. A retried item receives a diagnostic-specific instruction without the rejected value or item content. A terminal `provider_invalid_response` means finalization or schema validation was rejected; it does not prove child launch failure or assign sole fault to the model. An active caller/runtime cancellation signal is classified as `canceled`. A provider-originated typed/DOM abort without an active cancellation signal is retryable `provider_unavailable` with diagnostic `lcm_map_item_provider_aborted`; arbitrary exception message text containing `abort` is not cancellation evidence. Context-overflow, authentication, content-filter, API, timeout, and provider transport errors retain their appropriate terminal/retryable classifications. An unclassified internal item failure uses non-retryable `recovery_failed`; `invalid_request` remains reserved for request/pre-run validation. A model explicitly marked as lacking tool-call support is rejected before map creation with non-retryable `provider_invalid_response`.
