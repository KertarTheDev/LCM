...
| Local-provider deferred background capacity | Durable background LCM maintenance for a local Ollama or local OpenAI-compatible endpoint would overlap an active provider stream or queued foreground work | `provider_capacity_deferred` | `lcm.provider_capacity.deferred` | `retry` | `true` | `LcmMaintenanceResult.status = "deferred"` and/or content-safe event/metrics diagnostics. No provider request is sent. Root/task/explore streams wait in the foreground lane and agentic map-child streams wait in the lower-priority background lane instead of contacting the provider concurrently. Post-invocation timeout/capacity failures use their closest safe code, and connection failures use `provider_unavailable`, not `provider_capacity_deferred`. |
