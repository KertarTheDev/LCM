...
Map tools are durable asynchronous run tools. Initial `llm_map` and `agentic_map` calls do not accept `mapID`; they validate/register immutable input, create or resume the durable run, schedule work, and return the latest `LcmMapResult` snapshot without requiring all items to complete during the initial tool call. A classified local endpoint serializes physical provider calls and alternates foreground and background map turns when both lanes are queued; agentic children release capacity between model turns and tool work. Remote/unknown providers retain normal concurrency. Scheduling is automatic at creation, owning-session activation, and runtime recovery. `lcm_map_status` is observational and may idempotently ensure a run is registered with the scheduler, but polling is never required to trigger work. A future retry delay is registered as a runtime wake-up without invoking the provider early. `lcm_map_cancel` requests cancellation and returns the latest content-safe snapshot. `outputFileID` appears only after the run reaches `completed`.
