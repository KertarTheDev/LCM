...
Map tools are durable asynchronous run tools. Initial `llm_map` and `agentic_map` calls do not accept `mapID`; they validate/register immutable input, create or resume the durable run, schedule work, and return the latest `LcmMapResult` snapshot without requiring all items to complete during the initial tool call. A classified local endpoint serializes physical provider streams but alternates foreground and map-child turns when both lanes are queued; an agentic child never reserves the endpoint across tool work. Remote/unknown providers retain normal concurrency. If a run finishes before its initial call returns, the snapshot may be `completed`; otherwise callers retain its `mapID` and use `lcm_map_status` to poll no sooner than `retryAfterMs`. Status and normal owning-session activation may reconstruct either map kind from persisted identity, reconcile stale claims, and schedule it. A future retry delay is registered as a runtime wake-up without invoking the provider early. `lcm_map_cancel` requests cancellation and returns the latest content-safe snapshot. `outputFileID` appears only after the run reaches `completed`.
