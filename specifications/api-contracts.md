...
Prompt and command dispatch failures continue to use the normal extension-to-webview `sendMessageFailed` message so the composer draft and file attachments are restored through the existing prompt input recovery path. Advisory extension-side LCM prewarm failures must not produce `sendMessageFailed`. Prompt and command sends may perform a bounded readiness wait before submitting the SDK prompt/command request so transient retryable startup states can clear; while waiting, the extension uses the existing `sessionStatus: retry` shape and must honor user abort. Owner-lock readiness failures are not retried as a countdown loop; the extension may attempt non-forced stale-lock recovery first, and otherwise surfaces `sendMessageFailed` with the structured safe error plus a chat recovery affordance that calls the runtime-owned recover-lock route with explicit `force` only after user confirmation. If readiness remains blocked or the prompt/command dispatch itself fails with a structured runtime `LcmSafeError`, the payload may include optional `safeError?: LcmSafeError` preserving the runtime code, retryability, action, safe message, and diagnostic code. The webview must prefer `safeError.safeMessage` for user-facing memory recovery copy and must not expose raw DB paths or memory content.
