...
Status: normative v7.7.3 tree and projection contract.
...
Before provider dispatch, hard preparation also measures the complete rendered request. If it still exceeds usable
input after the configured tree target is met, preparation tightens its internal target by the measured excess and
repeats maintenance while durable frontier tokens strictly decrease. This accounts for request framing and fixed
upstream input without changing the public threshold or protected history. Cancellation, an invalid projection, or
no further reduction stops the loop; a request that still cannot fit fails closed.
...
prior summaries are not conflated. Both leaf and higher-level transformations extract useful continuation memory,
not a generic conversation recap: user intent, requirements and preferences, decisions and their rationale, exact
working details, verified progress, remaining work, and unresolved questions. Prompts distinguish preferences,
requirements, permissions, and observed outcomes, and ask for brief decisive source wording where paraphrasing could
strengthen user intent or completion status. Reference-data summaries preserve literal
...
every recovery primitive disabled and `StructuredOutput` required. The same
...
synthesis instruction, while the complete transcript remains available for provenance and conflicts. An already-valid
bounded answer is preserved; clipped, conflicting, or incomplete evidence deterministically downgrades claimed full
coverage to partial and adds the host-tracked gaps instead of triggering a lossy re-synthesis. Unscoped,
summary-scoped, and complete single-unit exact `lcm_expand_query` calls may
atomically reserve from the configured private logical-inference budget over the trusted
original-request-plus-focused-scope assignment and selected excerpts; clipped exact scopes return deterministic
evidence. This makes hard aggregation use clean bounded contexts
...
an at-most 65,536-character cumulative repair ledger. The repair session receives only the bounded original request,
focused scope, and that ledger, never the research child's raw tool transcript; a bounded recoverable synthesis may be
one labeled ledger section. It gets the configured number of
repair attempts. Host-verified citations prove exact prior-turn bytes and bounds, not semantic entailment, ordering, or
completeness, so parent guidance reconciles them with independently supported active-context facts. Those attempts use
structured output except that the last of two or more has no generated tool and
