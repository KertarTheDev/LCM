...
This document describes the current public route and VSCode/webview integration for LCM on the v7.4.15 base.
...
The extension forwards content-safe `lcm.*` envelopes only for the active or otherwise tracked session as `lcmEvent`. The main chat header hydrates with `requestLcmStatus`, retains a per-session cache, accepts snapshots monotonically by `updatedAt`, and refreshes after reconnect, relevant LCM events, and transition to idle. Hard, raw, and backlog pressure is kept out of the compact title row and appears in the collapsible upstream token-usage details below the timeline. Hard is `activeTokens / hardLimit`; raw and backlog use `softThreshold`; output reserve is already reflected by the runtime budget and is not subtracted again in the UI.
...
`TaskTimeline.tsx` requests the same bounded session activity projection and timestamp-merges provider-backed maintenance, expand-query, exploration, and map request bars with ordinary transcript bars. Non-provider maintenance-status bookkeeping does not create a zero-token bar. Provider calls with omitted usage remain visible as `Token usage unavailable`; reported input/output/cache values, provider/model identity, and cost status use one consistent tooltip shape. LCM activity reuses the upstream timeline palette and fixed-width/proportional-height geometry, but has no transcript scroll target. Both the settings card and timeline correlate response IDs so an out-of-order request for a previously focused session is ignored.
