...
- `client.session.lcm.db.recoverLock(...)`
...
Settings request, update, queued-maintenance cancel, DB diagnose, DB lock recovery, DB rebuild, and prompt-export envelopes carry webview-generated `requestID` values. The Memory settings tab tracks the current pending read, write, cancel, diagnose, recover-lock, rebuild, and export request IDs and ignores stale or out-of-order `.result` messages. Starting a settings update or prompt export invalidates any in-flight read. Maintenance cancel, DB diagnosis, DB lock recovery, DB rebuild, and prompt export block concurrent settings reads/writes until the result arrives. Metrics-driven refreshes do not start while a DB-backed write/cancel/diagnose/recover-lock/rebuild/export is pending, and a session switch resets pending request state before loading the newly focused session.
...
- safe action buttons for supported actions: retry/check-again refreshes, queued maintenance retry cancellation, read-only DB diagnostics for DB lock/corruption/unavailable states, guided owner-lock recovery preview/apply for recoverable stale locks, guided DB repair preview/apply for corrupt or unavailable diagnostics, new-task recovery, and contact-support links. Guided lock recovery and repair use only runtime-owned session routes, start with dry-run previews, refuse healthy or live-owner family state, and do not expose raw memory or direct database ownership in the webview.
