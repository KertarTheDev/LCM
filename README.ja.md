...
KiloCode-LCM は、長いセッションの扱いを改善するために作られた Kilo Code の改変版です。

通常のコーディングエージェントは、いずれチャット履歴を入れる余地がなくなります。そうなると、多くの場合、古い会話を短い要約に圧縮し、細かい情報を捨てます。これでチャットは続けられますが、以前のコマンド、ファイル、エラー、判断などの正確な内容をエージェントが忘れてしまうことがあります。

LCM は別の方法を使います。会話のローカルメモリを保持し、必要に応じて古い部分を要約し、あとから詳細を安全に探し直せる手段をエージェントに与えます。目標は単純です。長いセッションが、コンテキスト上限に達するたびに最初からやり直すように感じるのではなく、必要なときにノートを見返せる相手と作業しているように感じられることです。

改善を目指す点:

- 長いコーディング作業での継続性の向上。
- コンテキストが大きくなったときの偶発的な忘却の減少。
- 以前の正確な詳細をより確実に取り戻せること。
- 古いコンテキストが知らないうちに捨てられるケースの減少。

トークン使用量はタスクによって変わります。短いチャットでは、LCM が最近のコンテキストを新鮮に保ち、バックグラウンドでメモリ処理を行うことがあるため、ほぼ同じか少し多いトークンを使う場合があります。長いセッションでは、古いチャット履歴をメインプロンプトに何度も載せ続けるためのトークンは通常少なくなるはずですが、その一部はメモリのメンテナンスや検索呼び出しに使われることがあります。実際には、LCM はまずより安全に覚えておくことを優先し、その次に長いコンテキストで無駄になるトークンを減らすことを目指します。

これはまだプレリリースです。LCM を試すときは、このリポジトリの GitHub Release アーティファクトを使ってください。通常の Kilo Code Marketplace と npm パッケージは、引き続き upstream のリリースです。

LCM は論文 [LCM: Lossless Context Management](https://arxiv.org/abs/2605.04050) に着想を得ています。実装の詳細は [current-code specifications](specifications/README.md) にあります。
